 ...
 * 							   Movement Actions	 							*
 * 																			*
 * 	Contains functions for movement in the game world.						*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
 *																			*
...
char * const ship_dir_name[] = {
    "fore (n)", "starboard (e)", "aft (s)", "port (w)", "dorsal (u)", "ventral (d)",
    "fore-starboard (ne)", "fore-port (nw)", "aft-starboard (se)", "aft-port (sw)", "somewhere"
};
...
ch_ret move_char( CHAR_DATA *ch, EXIT_DATA *pexit, int fall )
{
	...
	{
		if((ch->pcdata && !IS_SET(ch->pcdata->act2, ACT_TRESPASS)) || !ch->pcdata)
		{
...
void do_build_walk( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_IMMORTAL( ch ) && IS_SET( ch->pcdata->flags, PCFLAG_BUILDWALK ) )
	{
		...
		if ( !xit )
		{
			...
			{
				send_to_char("&RYou cannot modify this room.\r\n", ch);
				return;
			}
			...
			{
				send_to_char
				( "&GYou cannot buildwalk anymore as this area has run out of vnums.\r\n",
						ch );
				return;
			}
...
void do_open( CHAR_DATA *ch, char *argument )
{
	...
	{
		/* 'open object' */
		if ( obj->item_type != ITEM_CONTAINER && !IS_SET( obj->wear_flags, ITEM_WEAR_CLOAK ) )
		{
			ch_printf( ch, "%s isn't a container.\n\r", capitalize( obj->short_descr ) );
			return;
		}
        if ( obj->item_type != ITEM_CONTAINER && obj->value[4] == 0 )
        {
            send_to_char("It would seem that is already open.\n\r", ch);
            return;
        }
        else if ( obj->item_type != ITEM_CONTAINER )
        {
            ch_printf( ch, "You open your %s.\n\r", obj->short_descr);
            act(AT_ACTION, "$n opens $s $p.", ch, obj, NULL, TO_ROOM );
            obj->value[4] = 0;
            return;
        }
...
void do_close( CHAR_DATA *ch, char *argument )
{
	...
	if ( ( obj = get_obj_here( ch, arg ) ) != NULL )
	{
        ...
        if ( obj->item_type != ITEM_CONTAINER && obj->value[4] == 1 )
        {
            send_to_char("That is already closed.\n\r", ch);
            return;
        }
        else if ( obj->item_type != ITEM_CONTAINER )
        {
            ch_printf( ch, "You close your %s.\n\r", obj->short_descr);
            act(AT_ACTION, "$n closes $s $p.", ch, obj, NULL, TO_ROOM);
            obj->value[4] = 1;
            return;
        } 
		if ( IS_SET(obj->value[1], CONT_CLOSED) )
...
void do_setlock( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char( "&RSyntax: setlock <direction> <old code> <new code>\n\r", ch );
        return;
    }
    ...
    if ( ( pexit = find_door( ch, arg1, TRUE ) ) != NULL )
    {
        ...
        if ( !IS_SET(pexit->exit_info, EX_KEYPAD) )
            { send_to_char( "The door doesn't have a keypad!\n\r",      ch ); return; }
        if ( pexit->key != atoi( arg2 ) )
            { send_to_char( "That is not the current code!\n\r",      ch ); return; }
        ...
        echo_to_room( AT_YELLOW, ch->in_room, "The keypad displays: 'Code changed'");

        if ( (pexit_rev = pexit->rexit) != NULL && pexit_rev->to_room == ch->in_room )
        {
            pexit_rev->key = atoi(arg3);
            echo_to_room( AT_YELLOW, pexit->to_room, "The keypad displays: 'Code changed'");
        }
    ...
    ch_printf( ch, "&RYou see no %s here.\n\r", arg1 );
    return;
}
	...
	if ( ( pexit = find_door( ch, arg, TRUE ) ) != NULL )
	{
		...
		{ send_to_char( "It can't be locked.\n\r",     ch ); return; }
        	...
        	else
	        {
        	    	if ( argument[0] == '\0' )
            		{
                	send_to_char("&RThis is a keypad controlled doorway.\n\r&RSyntax: LOCK <direction> <key code>\n\r", ch );
                 	return;
            	}

	        int code = atoi( argument );

        	if ( code != pexit->key )
            	{
                 	echo_to_room( AT_YELLOW, ch->in_room, "The keypad displays: 'Invalid access code'");
                 	if ( pexit->to_room )
                      	   echo_to_room( AT_YELLOW, pexit->to_room, "The keypad displays: 'Invalid access code'");
                 	   return;
            	}
        }
        if ( IS_SET(pexit->exit_info, EX_LOCKED) )
...
void do_unlock( CHAR_DATA *ch, char *argument )
{
	...
	if ( ( pexit = find_door( ch, arg, TRUE ) ) != NULL )
	{
        ...
        else
        {
            if ( argument[0] == '\0' )
            {
                 send_to_char("&RThis is a keypad controlled doorway.\n\r&RSyntax: UNLOCK <direction> <key code>\n\r", ch );
                 return;
            }
