 ...
 * 							  	  Utilities	 					   			*
 * 																			*
 *  A bunch of utility functions used in the other files.					*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
 *																			*
...
char *num_punct( int foo )
{
   ...
   for( index1 = index_new = 0; index1 < strlen( buf ); index1++, index_new++ )
   {
      if( index1 != 0 && ( index1 - rest ) % 3 == 0 )
      {
         buf_new[index_new] = ',';
         index_new++;
         buf_new[index_new] = buf[index1];
      }
      else
         buf_new[index_new] = buf[index1];
   }
