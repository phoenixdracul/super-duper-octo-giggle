 ...
 *                         													*
 * 							 Global Boards System	 					    *
 * 																			*
 *  Global boards which are readable from anywhere in the world.			*
 *  Each board can be restricted to players above a certain level or 		*
 *  only select players.													*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
...
//    { "Roleplay",    "Post your RP logs for rp-points", LEVEL_IMMORTAL, 1, DEF_INCLUDE, 60, NULL, FALSE },
    { "News",		"News of recent events",         0,     LEVEL_IMMORTAL, "all", DEF_NORMAL, 60, NULL, FALSE },
    { "Questions",	"FAQ's, Questions, Help",        0,     2,     "all", DEF_NORMAL, 60, NULL, FALSE },
    { "Ideas",   	"Post bugs and ideas",			 0, 	1,	   "all", DEF_NORMAL, 60, NULL, FALSE},
    { "Announce", 	"Announcements from Immortals",  0,     LEVEL_IMMORTAL, "all", DEF_NORMAL, 60, NULL, FALSE },
    { "Immortal",       "Immortal discussion",           LEVEL_IMMORTAL, LEVEL_IMMORTAL, "all", DEF_INCLUDE, 21, NULL, FALSE },
    { "Builders",	"Building and areas stuff",	     33,	33,	   "all", DEF_INCLUDE,  21,		NULL, FALSE},
    { "Coders", 	"Coding and Development", 		36, 	36,		"all", DEF_INCLUDE, 21, 	NULL, FALSE},
...
void finish_note ( GLOBAL_BOARD_DATA *board, NOTE_DATA *note )
{
    ...
    /* Echo that a note has been added to everyone who has read access to the board - Boran */
    DESCRIPTOR_DATA *cdesc;
    for(cdesc = first_descriptor; cdesc; cdesc = cdesc->next)
    {
    	if(cdesc->character && get_trust(cdesc->character) >= board->read_level)
    	{
    		if(str_cmp(board->short_name, "Personal") || gb_is_note_to(cdesc->character, note))
    			ch_printf(cdesc->character, "&R[&GGlobal Boards&R] &WA new note has been added to &G%s&W.&w\n\r",
    					board->short_name);
    	}
    }
}
...
void do_glist (CHAR_DATA *ch, char *argument)
{
    ...
    ch_printf ( ch, "&CNotes on the &W%s&C board&B:&W\n\r"
                   "&CNum&B: &CAuthor        Subject\n\r", ch->pcdata->board->short_name);
...
void handle_con_note_text (DESCRIPTOR_DATA *d, char * argument)
{
    ...
    /* Changed by Boran to allow lines of length < MAX_STRING_LENGTH	*/
    if (strlen (buf) > 253)
    {
        send_to_char_color ( "&RLine rejected - too long.&w\n\r",ch);
        return;
    }
