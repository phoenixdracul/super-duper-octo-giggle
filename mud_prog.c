 ...
 * 						  	   Mudprogs Module					    		*
 * 																			*
 *  Mudprogs for mobs, objects, rooms.										*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
 *																			*
		      ...
		      OBJ_DATA *obj, void *vo, CHAR_DATA *rndm )
{
  ...
  *pchck = '\0';
  ...
  if ( !str_cmp(chck, "ovnumwear") )
  {
	...
	if(actor)
	{
		for ( pObj = actor->first_carrying; pObj; pObj = pObj->next_content )
		  if ( pObj->wear_loc != WEAR_NONE && can_see_obj(actor, pObj) &&
...
//	}
	...
	if ( !str_cmp(chck, "wasinroom") )
	{
        ...
        {
            return IS_SET(chkchar->in_room->room_flags, ROOM_NO_MAGIC) ?  TRUE : FALSE;
        }
        if ( !str_cmp(chck, "safe") )
        {
            return IS_SET(chkchar->in_room->room_flags, ROOM_SAFE) ?  TRUE : FALSE;
        }

//        if ( !str_cmp(chck, "nosummon") )
//        {
//            return IS_SET(chkchar->in_room->room_flags, ROOM_NO_SUMMON) ?  TRUE : FALSE;
//        }
//        if ( !str_cmp(chck, "noastral") )
//        {
//            return IS_SET(chkchar->in_room->room_flags, ROOM_NO_ASTRAL) ?  TRUE : FALSE;
//        }
//        if ( !str_cmp(chck, "nosupplicate") )
//        {
//            return IS_SET(chkchar->in_room->room_flags, ROOM_NOSUPPLICATE) ?  TRUE : FALSE;
//        }
	if ( !str_cmp(chck, "norecall") )
	{
	    return IS_SET(chkchar->in_room->room_flags, ROOM_NO_RECALL) ? TRUE : FALSE;
	...
	if ( !str_cmp(chck, "mainclass") )
	{
	    return mprog_seval((char *)ability_name[chkchar->main_ability],
		opr, rval, mob);
	}
	if ( !str_cmp(chck, "secondclass") )
	{
	    return mprog_seval((char *)ability_name[chkchar->secondary_ability],
		opr, rval, mob);
	}
...
//	}
	...
	if (!str_cmp (chck, "isleader"))
	{
	    ...
	    ||	 mprog_seval(chkchar->name, opr, temp->number3, mob)
	    ||	 mprog_seval(chkchar->name, opr, temp->number4, mob)
	    ||	 mprog_seval(chkchar->name, opr, temp->number5, mob)
	    ||	 mprog_seval(chkchar->name, opr, temp->number6, mob)
	    ||   mprog_seval(chkchar->name, opr, temp->number7, mob) )
		return TRUE;
	    else
		return FALSE;
	}
...
*/
		if ( chkchar==obj->carried_by
		&&   obj->wear_loc > -1 
		&&  !str_cmp(rval, wear_locs[obj->wear_loc]) )
		    return TRUE;
	    }
	    return FALSE;
	}
	...
	if (!str_cmp (chck, "isclan2"))
	{
        ...
        if (!str_cmp (chck, "isclan3"))
        {
            ...
            if ( (temp = get_clan( rval )) == NULL )
                return FALSE;
            if ( mprog_seval(chkchar->name, opr, temp->number3, mob ) )
                return TRUE;
            else
                return FALSE;
        }
        if (!str_cmp (chck, "isclan4"))
        {
            CLAN_DATA *temp;
            if ( IS_NPC ( chkchar ) )
                return FALSE;
            if ( (temp = get_clan( rval )) == NULL )
                return FALSE;
            if ( mprog_seval(chkchar->name, opr, temp->number4, mob ) )
                return TRUE;
            else
                return FALSE;
        }
        if (!str_cmp (chck, "isclan5"))
        {
            CLAN_DATA *temp;
            if ( IS_NPC ( chkchar ) )
                return FALSE;
            if ( (temp = get_clan( rval )) == NULL )
                return FALSE;
            if ( mprog_seval(chkchar->name, opr, temp->number5, mob ) )
                return TRUE;
            else
                return FALSE;
        }
        if (!str_cmp (chck, "isclan6"))
        {
            CLAN_DATA *temp;
            if ( IS_NPC ( chkchar ) )
                return FALSE;
            if ( (temp = get_clan( rval )) == NULL )
                return FALSE;
            if ( mprog_seval(chkchar->name, opr, temp->number6, mob ) )
                return TRUE;
            else
                return FALSE;
        }
        if (!str_cmp (chck, "isclan7"))
        {
            CLAN_DATA *temp;
            if ( IS_NPC ( chkchar ) )
                return FALSE;
            if ( (temp = get_clan( rval )) == NULL )
                return FALSE;
            if ( mprog_seval(chkchar->name, opr, temp->number7, mob ) )
                return TRUE;
            else
                return FALSE;
        }
...
//	}
	if ( !str_cmp(chck, "rpp") )
	{
	    if ( IS_NPC(chkchar) )
		return FALSE;
	    return mprog_veval(chkchar->rppoints, opr, atoi(rval), mob);
	}
	if ( !str_cmp(chck, "hps") )
	{
...
int oprog_custom_trigger(char *command, char *argument, CHAR_DATA *ch)
{
  ...
  OBJ_DATA *vobj;
    ...
    if(!found)    
      return 0;
    set_supermob( vobj );
    mprog_driver( mprg->comlist, supermob, ch, vobj, NULL, NULL, FALSE );
    set_supermob( vobj );
  return 1;
}
...
int mprog_custom_trigger(char *command, char *argument, CHAR_DATA *ch)
{
  ...
  for ( vmob = room->first_person; vmob; vmob = vmob->next_in_room )
  {
    ...
    if(!found)
      return 0;
    mprog_driver( mprg->comlist, vmob, ch, NULL, NULL, NULL, FALSE );
  return 1;
}
...
int rprog_custom_trigger(char *command, char *argument, CHAR_DATA *ch)
{
  ...
  bool found = FALSE;
    ...
    if(!found)
      return 0;
    rset_supermob( room );
    mprog_driver( mprg->comlist, supermob, ch, NULL, NULL, NULL, FALSE );
    release_supermob();
  return 1;
}
