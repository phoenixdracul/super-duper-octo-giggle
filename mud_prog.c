          ...
          { 
            ...
            { 
              ...
              while ( ( start = strstr( dupl, word ) ) ) 
                ...
                { 
                        //skip the word where the "start" character(s) is(are) located -Matteo 
                        while( *start && !isspace(*start) ) start++; 
                        // if start exist, set new start point; else restart with new pc keywords 
                       if (*start) dupl = start+1; else break; 
                } 
            } 
          } 
      }
