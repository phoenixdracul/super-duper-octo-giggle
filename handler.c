 ...
 * 						    	Handler Module	 					    	*
 * 																			*
 *  Handles client-server interaction at a higher level than comm.c	and		*
 *  provides functions for handling characters.								*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
 *																			*
...
void room_explode_1( OBJ_DATA *obj , CHAR_DATA *xch, ROOM_INDEX_DATA *room , int blast )
{
	...
	for ( rch = room->first_person ; rch ;  rch = rnext )
	{
		...
		damage( rch, rch , dam, TYPE_EXPLOSION );
		if ( !char_died(rch) )
		{
			if ( IS_NPC( rch ) )
			{
...
int can_carry_n( CHAR_DATA *ch )
{
	...
	if ( get_eq_char(ch, WEAR_SHIELD) )
		++penalty;
	return URANGE(5, (ch->top_level+15)/5 + get_curr_dex(ch) + get_curr_str(ch) - 5 - penalty, 100);
}
...
void char_to_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
	...
	if( pRoomIndex != NULL && !get_room_index( pRoomIndex->vnum ) )
		pRoomIndex = NULL;
...
int apply_ac( OBJ_DATA *obj, int iWear )
{
	...
	case WEAR_BACK:	return	   obj->value[0];
        case WEAR_CLOAK: return  2*obj->value[0];
	}
...
void clean_mob( MOB_INDEX_DATA *mob )
{
	...
	xCLEAR_BITS(mob->act);	   mob->affected_by	= 0;
