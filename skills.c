 ...
 * 						      	 Skills File						    	*
 * 																			*
 *  Contains functions for checking if skills are useable, what damage 		*
 *  they do etc.															*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
 *																			*
...
void do_steal( CHAR_DATA *ch, char *argument )
{
	...
	if(IS_SET(obj->extra_flags, ITEM_UNIQUE))
	{
		...
		for ( pObj = ch->first_carrying; pObj; pObj = pObj->next_content )
		{
			...
			{
				send_to_char("You can only carry one of those. Don't steal, mkay.\n\r", ch);
				return;
			}
		}
	}
...
void do_hide( CHAR_DATA *ch, char *argument )
{
	...
	if( ch->substate == 1)
	{
    	send_to_char( "&RYou can't hide while you're so involved in your work.\n\r", ch );
		return;
	}
...
void do_poison( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !IS_NPC( ch )
  &&  ch->pcdata->learned[gsn_poison] <= 0  )
    {
    send_to_char( "What do you think you are, an assassin?\n\r", ch );
    return;
    }
  ...
  one_argument( argument, arg );
    ...
    if ( obj->item_type != ITEM_WEAPON && obj->item_type != ITEM_FOOD && obj->item_type != ITEM_DRINK_CON )
    {
	...
	{
		send_to_char( "The drink container is empty.\n\r", ch);
		return;
	}
    if (obj->item_type == ITEM_WEAPON && obj->value[3] != WEAPON_VIBRO_BLADE && obj->value[3] != WEAPON_FORCE_PIKE )
    {
    ...
    {
		send_to_char( "It has already been poisoned.\n\r",  ch );
		return;
    }
    /* Now we have a valid weapon, food or drink...check to see if we have the powder. */
    for ( pobj = ch->first_carrying; pobj; pobj = pobj->next_content )
    {
    ...
    /* Okay, we have the powder...do we have water? Only needed if we are poisoning weapons*/
    ...
    if ( !wobj && obj->item_type == ITEM_WEAPON)
    {
		send_to_char( "You have no water to mix with the poison.\n\r", ch );
		return;
    }
    ...
    WAIT_STATE( ch, skill_table[gsn_poison]->beats );
	...
	if ( obj->item_type == ITEM_WEAPON)
		separate_obj( wobj );
    if ( !IS_NPC( ch )
    && percent > ch->pcdata->learned[gsn_poison] )
    {
		set_char_color( AT_RED, ch );
		send_to_char( "You failed and spill some on yourself.  Ouch!\n\r", ch );
		set_char_color( AT_GREY, ch );
		damage( ch, ch, ch->skill_level[ASSASSIN_ABILITY] , gsn_poison );
	...
	{
		act(AT_RED, "You poison $p with a deadly dose of $P!", ch, obj, pobj, TO_CHAR);
		act(AT_RED, "$n poisons $p with $P!", ch, obj, pobj, TO_ROOM);
	}
	else if(obj->item_type == ITEM_DRINK_CON)
	{
		act(AT_RED, "You sprinkle a deadly amount of $p into $P!", ch, pobj, obj, TO_CHAR);
		act(AT_RED, "$n sprinkles some $p into $P.", ch, pobj, obj, TO_ROOM);
	}
    SET_BIT( obj->extra_flags, ITEM_POISONED );
	
    obj->cost *= ch->skill_level[ASSASSIN_ABILITY]/2;
	
	if(obj->item_type == ITEM_WEAPON)
	{
		/* Set an object timer.  Don't want proliferation of poisoned weapons */
		obj->timer = 10 + ch->skill_level[ASSASSIN_ABILITY] ;
	...
	if(wobj && obj->item_type == ITEM_WEAPON)
		extract_obj( wobj );
	learn_from_success( ch, gsn_poison );
	return;
}
