		...
		if ( number <= 1 && str_cmp( arg1, "all" ) && str_prefix( "all.", arg1 ) )
		{
			...
			if(IS_SET(obj->extra_flags, ITEM_UNIQUE))
			{
				...
				for ( pObj = ch->first_carrying; pObj; pObj = pObj->next_content )
				{
					if ( can_see_obj(ch, pObj) && pObj->pIndexData->vnum == obj->pIndexData->vnum )
					{
						send_to_char("You can only carry one of those.\n\r", ch);
						return;
					}
				}
			}*/
...
void do_holster( CHAR_DATA *ch, char *argument)
{
	...
	OBJ_DATA *holster;
	OBJ_DATA *weapon;
	OBJ_DATA *weapon2;	// Used if player is dual-wielding.
	...
	{
		send_to_char("&wYou are not wielding a weapon.\n\r", ch);
		return;
	}
	else
		weapon = get_eq_char(ch, WEAR_WIELD);

	if(!get_eq_char(ch, WEAR_HOLSTER_L)
			&& !get_eq_char(ch, WEAR_HOLSTER_R))
	{
		send_to_char("&wYou are not wearing a holster.\n\r", ch);
		return;
	}

	if(get_eq_char(ch, WEAR_DUAL_WIELD))	// If player is dual-wielding.
	{
		weapon2 = get_eq_char(ch, WEAR_DUAL_WIELD);
		if(!get_eq_char(ch, WEAR_HOLSTER_L))
		{
			if(get_eq_char(ch, WEAR_HOLSTER_R))
			{
				if(get_eq_char(ch, WEAR_HOLSTER_R)->first_content)
				{
					send_to_char("&wThe holster is full.\n\r", ch);
					return;
				}
				else
					holster = get_eq_char(ch, WEAR_HOLSTER_R);
			}
		}
		else
		{
			holster = get_eq_char(ch, WEAR_HOLSTER_L);
			if(holster->first_content)
			{
				if(get_eq_char(ch, WEAR_HOLSTER_R))
				{
					if(get_eq_char(ch, WEAR_HOLSTER_R)->first_content)
					{
						send_to_char("&wThe holsters are full.\n\r", ch);
						return;
					}
					else
						holster = get_eq_char(ch, WEAR_HOLSTER_R);
		...
		weapon2 = obj_to_obj(weapon2, holster);
		act(AT_PLAIN, "You holster $p.", ch, weapon2, NULL, TO_CHAR);
		act(AT_PLAIN, "$n holsters $p.", ch, weapon2, NULL, TO_ROOM);
		save_char_obj(ch);
	}

	if(!get_eq_char(ch, WEAR_HOLSTER_L))	// Now do the primary weapon.
	{
		if(get_eq_char(ch, WEAR_HOLSTER_R))
		{
	...
	weapon = obj_to_obj(weapon, holster);
	act(AT_PLAIN, "You holster $p.", ch, weapon, NULL, TO_CHAR);
	act(AT_PLAIN, "$n holsters $p.", ch, weapon, NULL, TO_ROOM);
	save_char_obj(ch);
	return;
}
...
void do_draw( CHAR_DATA *ch, char *argument)
{
	...
	// Check if player has hands with which he can wield.
	if( get_eq_char(ch, WEAR_WIELD) || get_eq_char(ch, WEAR_HOLD))
	{
		if( !can_dual( ch ) )
		{
			send_to_char("&wYour hands are full.\n\r", ch);
			return;
		}
	}
	// Check is player is wearing a holster.
	if(!get_eq_char(ch, WEAR_HOLSTER_L)
			&& !get_eq_char(ch, WEAR_HOLSTER_R))
	{
		send_to_char("&wYou aren't wearing a holster.\n\r", ch);
		return;
	}
	...
	if(arg1[0] != '\0')
	{
		if(get_eq_char(ch, WEAR_HOLSTER_L))
		{
			weapon = get_obj_list( ch, arg1, get_eq_char(ch, WEAR_HOLSTER_L)->first_content );
			...
			{
				// Weapon not found in the holster. Search the other one.
				weapon = get_obj_list( ch, arg1, get_eq_char(ch, WEAR_HOLSTER_R)->first_content );
				if(!weapon || weapon->item_type != ITEM_WEAPON)
				{
					send_to_char("&wI don't see a weapon like that.\n\r", ch);
				}
				else
					weapon_found = TRUE;
			}
		}
		else if(get_eq_char(ch, WEAR_HOLSTER_R))
		{
			weapon = get_obj_list( ch, arg1, get_eq_char(ch, WEAR_HOLSTER_R)->first_content );
			if(!weapon || weapon->item_type != ITEM_WEAPON)
			{
	...
	else	// No arguments. Grip any weapon we can find in the holsters.
	{
		if(get_eq_char(ch, WEAR_HOLSTER_L))
		{
			// Assume that we'll only have one weapon in each holster.
			weapon = get_eq_char(ch, WEAR_HOLSTER_L)->first_content;
			if ( weapon && weapon->item_type == ITEM_WEAPON)
				weapon_found = TRUE;
			else
				weapon = FALSE;
		...
		if(get_eq_char(ch, WEAR_HOLSTER_R))
		{
			if(weapon_found)	// if a weapon's already been found.
			{
				second_weapon = get_eq_char(ch, WEAR_HOLSTER_R)->first_content;
				if(second_weapon && second_weapon->item_type == ITEM_WEAPON)
					dual_found = TRUE;
			}
			else
			{
				weapon = get_eq_char(ch, WEAR_HOLSTER_R)->first_content;
				if ( weapon && weapon->item_type == ITEM_WEAPON)
					weapon_found = TRUE;
			}
		}
	...
	if(weapon_found)
	{
		if ( (ch->carry_number + get_obj_number( weapon ) > can_carry_n( ch )))
		{
			act( AT_PLAIN, "$d: you can't carry that many items.",
					ch, NULL, weapon->name, TO_CHAR );
			return;
		}
		if ( ch->carry_weight + get_obj_weight(weapon) > can_carry_w( ch ) )
		{
			act( AT_PLAIN, "$d: you can't carry that much weight.",
					ch, NULL, weapon->name, TO_CHAR );
			return;
		...
		obj_from_obj( weapon );
		weapon = obj_to_char( weapon, ch );

		act(AT_PLAIN, "You draw $p from a holster.", ch, weapon, NULL, TO_CHAR);
		act(AT_PLAIN, "$n draws $p from a holster.", ch, weapon, NULL, TO_ROOM);
		wear_obj(ch, weapon, TRUE, -1);
	}
	else
	{
		act(AT_PLAIN, "You don't have a holstered weapon.", ch, NULL, NULL, TO_ROOM);
		return;
	}
	...
	if(dual_found)
	{
		...
		obj_from_obj( second_weapon );
		second_weapon = obj_to_char( second_weapon, ch );

		act(AT_PLAIN, "You draw $p from a holster.", ch, second_weapon, NULL, TO_CHAR);
		act(AT_PLAIN, "$n draws $p from a holster.", ch, second_weapon, NULL, TO_ROOM);
		wear_obj(ch, second_weapon, TRUE, -1);
	}
	save_char_obj(ch);
}
...
void do_put( CHAR_DATA *ch, char *argument )
{
	...
	if ( number <= 1 && str_cmp( arg1, "all" ) && str_prefix( "all.", arg1 ) )
	{
		...
		if(IS_SET(container->wear_flags, ITEM_WEAR_HOLSTER1) || IS_SET(container->wear_flags, ITEM_WEAR_HOLSTER2))
		{
			if(obj->item_type != ITEM_WEAPON)
			{
				send_to_char("Holsters are for weapons.\n\r", ch);
				return;
			}
			else if(container->first_content)
			{
				send_to_char("That holster is already full.\n\r", ch);
				return;
			}
		}
		...
		if(IS_SET(obj->extra_flags, ITEM_UNIQUE))
		{
			send_to_char("You can't put those types of items in containers.\n\r", ch);
			return;
		}
	...
	else
	{
		...
		if(IS_SET(container->wear_flags, ITEM_WEAR_HOLSTER1) || IS_SET(container->wear_flags, ITEM_WEAR_HOLSTER2))
		{
			send_to_char("You can't put everything into a holster!\n\r", ch);
			return;
		}
...
void do_give( CHAR_DATA *ch, char *argument )
{
	...
	/* Don't allow players to give items to another player's vendor */
	if ( IS_NPC(victim) && victim->pIndexData->pShop)
	{
		if((victim->pIndexData->pShop->playershop == TRUE
				&& str_cmp(ch->name, victim->pIndexData->pShop->owner))
				|| (!ch->pcdata->clan_name && victim->pIndexData->pShop->clanshop == TRUE)
				|| (victim->pIndexData->pShop->clanshop == TRUE
						&& str_cmp(ch->pcdata->clan_name, victim->pIndexData->pShop->owner)))
		{
			do_emote(victim, "shakes his head.");
			do_say(victim, "No, thank you.");
			return;
		}
		else if(victim->pIndexData->pShop->owner
				&& ((victim->pIndexData->pShop->playershop &&!str_cmp(ch->name, victim->pIndexData->pShop->owner))
						|| (ch->pcdata && ch->pcdata->clan_name && victim->pIndexData->pShop->clanshop && !str_cmp(ch->pcdata->clan_name, victim->pIndexData->pShop->owner))))
		{
			/* Remove the inventory flag for pshops. */
	...
	/* If victim is a player or clan shop, save the vendor's inventory.	*/
	if(IS_NPC(victim) && victim->pIndexData->pShop
			&& (victim->pIndexData->pShop->clanshop ||
					victim->pIndexData->pShop->playershop))
	{
		save_vendor(victim->in_room);
	}
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, BOOL fReplace, sh_int wear_bit )
{
	 ...
	 */
	 ...
	 case ITEM_WEAR_BACK:
		 ...
		 oprog_wear_trigger( ch, obj );
		 return;
    case ITEM_WEAR_CLOAK:
        if ( !can_layer( ch, obj, WEAR_CLOAK ) )
        {
            send_to_char( "It won't fit overtop of what you're already wearing.\n\r", ch );
            return;
        }
