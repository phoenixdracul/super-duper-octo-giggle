 ...
 *                         													*
 * 						    Bounty Hunter Functions 					    *
 * 																			*
 *  Functions, commands and skills for the bounty hunter class.				*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
 *																			*
...
//	if(aff2_by())	// TODO
//	{
//		send_to_char("&RPeople are still weary after your last ambush.\n\r", ch);
//		return;
//	}
	...
	if ( !IS_AWAKE(victim) || percent < ch->pcdata->learned[gsn_ambush] ){
		...
		/* Cooldown timer added by Boran */ // TODO
//		af.type = gsn_ambush;
//		af.duration = number_range(15, 25);
//		af.bitvector = AFF_NOAMBUSH;
...
void do_trespass(CHAR_DATA *ch, char *argument)
{
	...
	if(IS_NPC(ch))
	{
		send_to_char("&RMobs cannot trespass!\n\r", ch);
		return;
	}
	if(arg[0] == '\0')
	{
		send_to_char("&RWhich home do you want to break into?\n\r", ch);
		return;
	}
	...
	if(!ch->in_room)
	{
		send_to_char("&RYou must be in a room first.\n\r", ch);
		return;
	}
	exit = get_exit(ch->in_room, edir);

	/* Invalid exit */
	if(!exit || !exit->to_room)
	{
		send_to_char("&RThere's nothing in that direction!\n\r", ch);
		return;
	}
	...
	/* Don't allow character to trespass while fighting. */
	if ( ch->fighting )
	{
		send_to_char("&RYou're a bit busy fighting right now...\n\r", ch);
	}

	/* If the room isn't private, simply allow the character to move in.. */
	if (!room_is_private(ch, exit->to_room ))
	{
		send_to_char("&wThere's no reason to break in there.\n\rYou stroll right in...\n\r", ch);
		move_char(ch, exit, 0);
		return;
	}

	int chance;
	chance = IS_NPC(ch) ? ch->top_level : (int) (ch->pcdata->learned[gsn_trespass]) ;

	// Skill fails...
	if(number_percent() > chance)
	{
		int half_chance = number_range(0, 1);
		if(half_chance == 0)
		{
			send_to_char("&RYou search and search, but are unable to find a way in.\n\r", ch);
			learn_from_failure(ch, gsn_trespass);
		}
		...
		if(half_chance == 1)
		{
			...
			for ( attempt = 0; attempt < 8; attempt++ )	// Try and find an exit 8 times.
			{
				door = number_door( );
				if ( ( fexit = get_exit(was_in, door) ) == NULL
						||   !fexit->to_room
						|| ( IS_SET(fexit->exit_info, EX_CLOSED)
								&&   !IS_AFFECTED( ch, AFF_PASS_DOOR ) )
								|| ( IS_NPC(ch)
										&&   IS_SET(fexit->to_room->room_flags, ROOM_NO_MOB) ) )
					continue;
				...
				send_to_char("&RYou begin to feel uneasy, and retreat before before you can be spotted!\n\r", ch);
				learn_from_failure(ch, gsn_trespass);
				move_char( ch, fexit, 0 );
				if ( ( now_in = ch->in_room ) == was_in )
					continue;
				f_exit_found = TRUE;
				ch->in_room = was_in;
				act( AT_FLEE, "$n runs off hastily!", ch, NULL, NULL, TO_ROOM );
				ch->in_room = now_in;
	...
	// Skill passes...
	send_to_char("&GYou manage to find a way in, and slip quietly through the entrance...\n\r", ch);

	if(!IS_SET(ch->affected_by, AFF_SNEAK))
		SET_BIT   ( ch->affected_by, AFF_SNEAK );

	if(!IS_SET(ch->pcdata->act2, ACT_TRESPASS))
		SET_BIT( ch->pcdata->act2, ACT_TRESPASS);

	learn_from_success(ch, gsn_trespass);

	// Move player, then remove sneak and trespassing bits.
	move_char( ch, exit, 0 );

	REMOVE_BIT(ch->affected_by, AFF_SNEAK);
	REMOVE_BIT(ch->pcdata->act2, ACT_TRESPASS);
	return;
}
