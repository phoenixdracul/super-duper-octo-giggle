 ...
 * 																			*
 *  A whole bunch of miscellaneous functions for players.					*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
 *																			*
...
void do_roulette_bet( CHAR_DATA*, char* );
void do_arena_bet(CHAR_DATA*, char* );
...
void do_bet( CHAR_DATA *ch, char *argument )
{
	...
	table = get_roulette_obj();
	if( ch->in_room == table->in_room )
		do_roulette_bet(ch, argument);
	else if(in_start_arena || ppl_challenged)
		do_arena_bet(ch, argument);
	else
		ch_printf(ch, "&WFind a roulette table or wait for the arena to open.\n\r"
				"&CRoulette bets&W: &cbet <amount> on <1-36, black, red, doz1, col1...>\n\r"
				"&CArena bets&W: &cbet <player> <amount>\n\r");
}
...
void do_use( CHAR_DATA *ch, char *argument )
{
	...
	{
		OBJ_DATA *temp_dev;
		temp_dev = get_obj_carry(ch, argd );
		if(temp_dev && temp_dev->item_type == ITEM_LIGHT)
		{
			nifty_is_name(argd, temp_dev->name);
			do_wear(ch, argd);
			return;
		}
		do_takedrug( ch , argd );
...
void do_setcolor( CHAR_DATA *ch, char *argument)
{
	...
	if((str_cmp(arg, "emote") && str_cmp(arg, "speech")) || argument[0] == '&' || strlen(argument) != 1 ||
			(argument[0] != 'x' && argument[0] != 'r' &&
					argument[0] != 'g' && argument[0] != 'O' &&
					argument[0] != 'b' && argument[0] != 'p' &&
					argument[0] != 'c' && argument[0] != 'w' &&
					argument[0] != 'z' && argument[0] != 'R' &&
					argument[0] != 'G' && argument[0] != 'Y' &&
					argument[0] != 'B' && argument[0] != 'P' &&
					argument[0] != 'C' && argument[0] != 'W' &&
					(!is_number(argument) || atoi(argument) > 255 || atoi(argument) < 0)))
	{
		send_to_char( "&RSyntax: setcolor emote/speech <color code> &WHELP TEXTCOLOR for more info.&w\r\n", ch );
		return;
	}
	char colorstr[10];
/*	colorstr[0] = '&';
	colorstr[1] = argument[0];
	colorstr[2] = '\0'; */
	if (is_number(argument))
		sprintf(colorstr, "&%d", atoi(argument));
	else
		sprintf(colorstr, "&%c", argument[0]);

	if(!str_cmp(arg, "speech"))
	{
		if(ch->textcolor)
			DISPOSE(ch->textcolor);
		ch->textcolor = str_dup(colorstr);
		send_to_char( "&GSpeech color set.\r\n", ch );
	}
	else if(!str_cmp(arg, "emote"))
	{
		if(ch->emotecolor)
			DISPOSE(ch->emotecolor);
		ch->emotecolor = str_dup(colorstr);
		send_to_char( "&GEmote color set.\n\r", ch);
	}
	return;
}
...
void do_drink( CHAR_DATA *ch, char *argument )
{
	...
	case ITEM_DRINK_CON:
	   ...
	   too I suppose... sheesh! */
		...
		if ( obj->value[3] || IS_SET(obj->extra_flags, ITEM_POISONED))
		{
...
void do_rpconvert( CHAR_DATA *ch, char *argument )
{
	...
	int total_points;
	if(IS_NPC(ch)) return;
	total_points = ch->rppoints;
	if(ch->desc && ch->desc->account && ch->desc->account->points)
		total_points += ch->desc->account->points;
...
void do_arm( CHAR_DATA *ch, char *argument )
{
	...
	if(obj->timer > 0 )
	{
		send_to_char("It's already armed!\n\r", ch );
		return;
	}
	...
	if ( timer < 0 )
	{
		send_to_char( "The timer must be set at least to 0.\n\r", ch );
		return;
	}

	if ( timer > 5 )
	{
		send_to_char( "The timer must be lower than 5.\n\r", ch );
		return;
	}
...
void tfold_area( AREA_DATA * tarea, const char *filename, bool install )
{
   ...
   for( vnum = tarea->low_o_vnum; vnum <= tarea->hi_o_vnum; vnum++ )
   {
      ...
      fprintf( fpout, "~\n" );
      fprintf( fpout, "%d\n", pObjIndex->layers ? pObjIndex->layers : 0 );
   ...
   for( vnum = tarea->low_r_vnum; vnum <= tarea->hi_r_vnum; vnum++ )
   {
      ...
      for( xit = room->first_exit; xit; xit = xit->next )
      {
         ...
         if( xit->distance > 1 )
            fprintf( fpout, "1 %d %d %d 0 0 %d\n", xit->key, xit->vnum, xit->distance, xit->keypad );
         else
            fprintf( fpout, "0 %d %d 0 0 %d\n", xit->key, xit->vnum, xit->keypad );
      }
      ...
      for( pReset = room->first_reset; pReset; pReset = pReset->next )
      {
         switch ( pReset->command ) /* extra arg1 arg2 arg3 */
         {
            ...
            case 'o':
            case 'O':
               fprintf( fpout, "R %c %d %d %d %d -1 -1 -1\n", UPPER( pReset->command ),
   ...
   for( vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++ )
   {
      ...
      fprintf( fpout, " %d   %2d %2d %2d %2d %2d   %3d %3d",
               pShop->keeper,
               pShop->buy_type[0],
               pShop->buy_type[1], pShop->buy_type[2], pShop->buy_type[3], pShop->buy_type[4], pShop->profit_buy,
               pShop->profit_sell );
...
void do_tsavearea( CHAR_DATA *ch, char *argument )
{
   AREA_DATA *tarea;
   char filename[256];
...
/* Ensure not wiping out their area with save before load - Scryn 8/11 */
   ...
   sprintf( filename, "%s%s.trelar", BUILD_DIR, tarea->filename );
   tfold_area( tarea, filename, FALSE );
   send_to_char( "Done.\r\n", ch );
}
