 ...
 * 							     Clans File									*
 * 																			*
 * 	Contains stuff used for clans. Doh.										*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
 *																			*
...
void free_clan( CLAN_DATA * clan )
{
	...
	STRFREE( clan->number5 );
	STRFREE( clan->number6 );
	STRFREE( clan->number7 );
	STRFREE( clan->atwar );
	STRFREE( clan->tmpstr );
	DISPOSE( clan );
}
...
void save_clan( CLAN_DATA *clan )
{
	...
	else
	{
		...
		fprintf( fp, "NumberFive   %s~\n",	clan->number5		);
		fprintf( fp, "NumberSix    %s~\n",	clan->number6		);
		fprintf( fp, "NumberSeven  %s~\n",	clan->number7		);
		fprintf( fp, "PKills       %d\n",	clan->pkills		);
	...
	for ( ; ; )
	{
		...
		case 'E':
			if ( !str_cmp( word, "End" ) )
			{
				...
				if (!clan->number5)
					clan->number5		= STRALLOC( "" );
				if (!clan->number6)
					clan->number6		= STRALLOC( "" );
				if (!clan->number7)
					clan->number7		= STRALLOC( "" );
				if (!clan->tmpstr)
		...
		case 'N':
			...
			KEY( "NumberFive",	clan->number5,		fread_string( fp ) );
			KEY( "NumberSix",	clan->number6,		fread_string( fp ) );
			KEY( "NumberSeven",	clan->number7,		fread_string( fp ) );
			break;
...
void do_induct( CHAR_DATA *ch, char *argument )
{
	...
	if ( (ch->pcdata && ch->pcdata->bestowments
			...
			||   !str_cmp( ch->name, clan->number3 )
			||   !str_cmp( ch->name, clan->number4 )
			||   !str_cmp( ch->name, clan->number5 )
			||   !str_cmp( ch->name, clan->number6 )
			||   !str_cmp( ch->name, clan->number7 ) )
		;
	else
	{
		send_to_char( "Huh?\n\r", ch );
...
bool can_outcast( CLAN_DATA *clan, CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if( !str_cmp( victim->name, clan->number2 ) )
		return FALSE;
	if( !str_cmp( ch->name, clan->number3 ) )
		return TRUE;
	if( !str_cmp( victim->name, clan->number3 ) )
		return FALSE;
	if( !str_cmp( ch->name, clan->number4 ) )
		return TRUE;
	if( !str_cmp( victim->name, clan->number4 ) )
		return FALSE;
	if( !str_cmp( ch->name, clan->number5 ) )
		return TRUE;
	if( !str_cmp( victim->name, clan->number5 ) )
		return FALSE;
	if( !str_cmp( ch->name, clan->number6 ) )
		return TRUE;
	if( !str_cmp( victim->name, clan->number6 ) )
		return FALSE;
	if( !str_cmp( ch->name, clan->number7 ) )
		return TRUE;
	if( !str_cmp( victim->name, clan->number7 ) )
		return FALSE;
	return TRUE;
}
...
void do_outcast( CHAR_DATA *ch, char *argument )
{
	...
	if ( !str_cmp( victim->name, ch->pcdata->clan->number3 ) )
	{
		STRFREE( ch->pcdata->clan->number3 );
		ch->pcdata->clan->number3 = STRALLOC( "" );
	}
	if ( !str_cmp( victim->name, ch->pcdata->clan->number4 ) )
	{
		STRFREE( ch->pcdata->clan->number4 );
		ch->pcdata->clan->number4 = STRALLOC( "" );
	}
	if ( !str_cmp( victim->name, ch->pcdata->clan->number5 ) )
	{
		STRFREE( ch->pcdata->clan->number5 );
		ch->pcdata->clan->number5 = STRALLOC( "" );
	}
	if ( !str_cmp( victim->name, ch->pcdata->clan->number6 ) )
	{
		STRFREE( ch->pcdata->clan->number6 );
		ch->pcdata->clan->number6 = STRALLOC( "" );
	}
	if ( !str_cmp( victim->name, ch->pcdata->clan->number7 ) )
	{
		STRFREE( ch->pcdata->clan->number7 );
		ch->pcdata->clan->number7 = STRALLOC( "" );
	}
...
void do_setclan( CHAR_DATA *ch, char *argument )
{
	...
	if ( arg1[0] == '\0' )
	{
		...
		send_to_char( "\n\rField being one of:\n\r", ch );
		send_to_char( " leader number1 - number7 subclan\n\r", ch );
	...
	if ( !strcmp( arg2, "number3" ) )
	{
		STRFREE( clan->number3 );
		clan->number3 = STRALLOC( argument );
		send_to_char( "Done.\n\r", ch );
		save_clan( clan );
		return;
	}
	if ( !strcmp( arg2, "number4" ) )
	{
		STRFREE( clan->number4 );
		clan->number4 = STRALLOC( argument );
		send_to_char( "Done.\n\r", ch );
		save_clan( clan );
		return;
	}
	if ( !strcmp( arg2, "number5" ) )
	{
		STRFREE( clan->number5 );
		clan->number5 = STRALLOC( argument );
		send_to_char( "Done.\n\r", ch );
		save_clan( clan );
		return;
	}
	if ( !strcmp( arg2, "number6" ) )
	{
		STRFREE( clan->number6 );
		clan->number6 = STRALLOC( argument );
		send_to_char( "Done.\n\r", ch );
		save_clan( clan );
		return;
	}
	if ( !strcmp( arg2, "number7" ) )
	{
		STRFREE( clan->number7 );
		clan->number7 = STRALLOC( argument );
		send_to_char( "Done.\n\r", ch );
		save_clan( clan );
		return;
	}
...
void do_enlist( CHAR_DATA *ch, char *argument )
{
	...
	else if ( IS_SET( ch ->in_room->room_flags2, ROOM_H_RECRUIT) )
		clan = get_clan("The Hunters Guild");
	else
	{
...
void do_resign( CHAR_DATA *ch, char *argument )
{
	...
	if ( !str_cmp( ch->name, ch->pcdata->clan->number3 ) )
	{
		STRFREE( ch->pcdata->clan->number3 );
		ch->pcdata->clan->number3 = STRALLOC( "" );
	}
	if ( !str_cmp( ch->name, ch->pcdata->clan->number4 ) )
	{
		STRFREE( ch->pcdata->clan->number4 );
		ch->pcdata->clan->number4 = STRALLOC( "" );
	}
	if ( !str_cmp( ch->name, ch->pcdata->clan->number5 ) )
	{
		STRFREE( ch->pcdata->clan->number5 );
		ch->pcdata->clan->number5 = STRALLOC( "" );
	}
	if ( !str_cmp( ch->name, ch->pcdata->clan->number6 ) )
	{
		STRFREE( ch->pcdata->clan->number6 );
		ch->pcdata->clan->number6 = STRALLOC( "" );
	}
	if ( !str_cmp( ch->name, ch->pcdata->clan->number7 ) )
	{
		STRFREE( ch->pcdata->clan->number7 );
		ch->pcdata->clan->number7 = STRALLOC( "" );
	}
...
void do_appoint ( CHAR_DATA *ch , char *argument )
{
	...
	if ( argument[0] == '\0' )
	{
		send_to_char( "Useage: appoint <name> < first | second | etc. >\n\r", ch );
		return;
	}
	...
	else if ( !str_cmp( argument , "third" )  )
	{
		if ( ch->pcdata->clan->number3 && str_cmp( ch->pcdata->clan->number3 , "" ))
		{
			send_to_char( "You already have someone in that position ... demote them first.\n\r", ch );
			return;
		}
		...
		STRFREE( ch->pcdata->clan->number3 );
		arg[0] = UPPER(arg[0]);
		ch->pcdata->clan->number3 = STRALLOC( arg );
	}
	else if ( !str_cmp( argument , "fourth" )  )
	{
		if ( ch->pcdata->clan->number4 && str_cmp( ch->pcdata->clan->number4 , "" ))
		{
			send_to_char( "You already have someone in that position ... demote them first.\n\r", ch );
			return;
		}
		...
		STRFREE( ch->pcdata->clan->number4 );
		arg[0] = UPPER(arg[0]);
		ch->pcdata->clan->number4 = STRALLOC( arg );
	}
	else if ( !str_cmp( argument , "fifth" )  )
	{
		if ( ch->pcdata->clan->number5 && str_cmp( ch->pcdata->clan->number5 , "" ))
		{
			send_to_char( "You already have someone in that position ... demote them first.\n\r", ch );
			return;
		}
		...
		STRFREE( ch->pcdata->clan->number5 );
		arg[0] = UPPER(arg[0]);
		ch->pcdata->clan->number5 = STRALLOC( arg );
	}
	else if ( !str_cmp( argument , "sixth" )  )
	{
		if ( ch->pcdata->clan->number6 && str_cmp( ch->pcdata->clan->number6 , "" ))
		{
			send_to_char( "You already have someone in that position ... demote them first.\n\r", ch );
			return;
		}
		...
		STRFREE( ch->pcdata->clan->number6 );
		arg[0] = UPPER(arg[0]);
		ch->pcdata->clan->number6 = STRALLOC( arg );
	}
	else if ( !str_cmp( argument , "seventh" )  )
	{
		if ( ch->pcdata->clan->number7 && str_cmp( ch->pcdata->clan->number7 , "" ))
		{
			send_to_char( "You already have someone in that position ... demote them first.\n\r", ch );
			return;
		}
		...
		STRFREE( ch->pcdata->clan->number7 );
		arg[0] = UPPER(arg[0]);
		ch->pcdata->clan->number7 = STRALLOC( arg );
	}
	else do_appoint( ch , "" );
...
void do_demote ( CHAR_DATA *ch , char *argument )
{
	...
	if ( !str_cmp( argument , ch->pcdata->clan->number1 )  )
	{
        ...
        if ( !str_cmp( argument , ch->pcdata->clan->number2 )  )
	{
		send_to_char( "Player Demoted!", ch );
	 ...
	 if ( !str_cmp( argument , ch->pcdata->clan->number3 )  )
	{
		send_to_char( "Player Demoted!", ch );
	...
	if ( !str_cmp( argument , ch->pcdata->clan->number4 )  )
	{
		send_to_char( "Player Demoted!", ch );
	...
	if ( !str_cmp( argument , ch->pcdata->clan->number5 )  )
	{
		send_to_char( "Player Demoted!", ch );
	...
	if ( !str_cmp( argument , ch->pcdata->clan->number6 )  )
	{
		send_to_char( "Player Demoted!", ch );
	...
	if ( !str_cmp( argument , ch->pcdata->clan->number7 )  )
	{
		send_to_char( "Player Demoted!", ch );
...
void do_war ( CHAR_DATA *ch , char *argument )
{
	...
	{
		send_to_char( "Your clan hasn't empowered you to declare war!\n\r", ch );
		return;
	}
...
void do_empower ( CHAR_DATA *ch , char *argument )
{
	...
	{
		send_to_char( "Your clan hasn't seen fit to bestow that ability to you!\n\r", ch );
		return;
	}
	...
	else if ( !str_cmp( arg2, "clanshops" ) )
	{
		sprintf( buf, "%s %s", victim->pcdata->bestowments, arg2 );
		DISPOSE( victim->pcdata->bestowments );
		victim->pcdata->bestowments = str_dup( buf );
		ch_printf( victim, "%s has given you permission to handle clan shops.\n\r",
				ch->name );
		send_to_char( "Ok, they now have the ability to handle clan shops.\n\r", ch );
	}
	...
	else
	{
		...
		send_to_char( 	"clanshops:   ability to handle shops belonging to the clan\n\r", ch);
...
void do_clanstat( CHAR_DATA *ch, char *argument)
{
	...
	for ( planet = first_planet ; planet ; planet = planet->next )
		if ( planet->governed_by && clan == planet->governed_by )
		{
			support += planet->pop_support;
			pCount++;
			revenue += get_taxes(planet);
	...
	ch_printf(ch, "&C+===================================================================+&z\n\r");
	ch_printf(ch, "&C|&z     &BClan Name&W: %-20.20s &BPlanets&W: %-2.2d                   &C|&z\n\r", clan->name, pCount );
	ch_printf(ch, "&C|&z         &BFunds&W: %-9.9d             &BTroops&W: %-4.4d                 &C|&z\n\r", clan->funds, clan->troops );
	if( support >= 0 )
	...
	ch_printf(ch, "&C|&z        &BSecond&W: %-12.12s           &BThird&W: %-12.12s         &C|\n\r&z", clan->number2 ? clan->number2 : "none", clan->number3 ? clan->number3 : "none" );
        ch_printf(ch, "&C|&z        &BFourth&W: %-12.12s           &BFifth&W: %-12.12s         &C|\n\r&z", clan->number4 ? clan->number4 : "none", clan->number5 ? clan->number5 : "none" );
        ch_printf(ch, "&C|&z         &BSixth&W: %-12.12s         &BSeventh&W: %-12.12s         &C|\n\r&z", clan->number6 ? clan->number6 : "none", clan->number7 ? clan->number7 : "none" );
	ch_printf(ch, "&C+===================================================================+\n\r&z" );
	...
	if ( clan->first_subclan )
	{
		...
		{
			ch_printf(ch, "&x&W+---------------------------------------------------------+\n\r");
			ch_printf(ch, "&x&W| &B%-12.12s&W:                         &YFunds&W: %-9.9d  |\n\r", subclan->name ? subclan->name : "no name", subclan->funds);
			if( clan->leader[0] != 0 )
				ch_printf(ch, "&x&W|       &BLeader&W: %-12.12s       &BSpacecraft&W: %-2.2d         |\n\r", subclan->leader ? subclan->leader : "none", subclan->spacecraft );
			else
				ch_printf(ch, "&x&W|       &BLeader&W: None.              &BTroops&W: %-2.2d             |\n\r", subclan->troops );
			ch_printf(ch, "&x&W|        &BFirst&W: %-12.12s         &BSpacecraft&W: %-2.2d       |\n\r", subclan->number1 ? subclan->number1 : "none", subclan->spacecraft );
			ch_printf(ch, "&x&W|       &BSecond&W: %-12.12s          &BMembers&W: %-2.2d         |\n\r", subclan->number2 ? subclan->number2 : "none", subclan->members );
			ch_printf(ch, "&x&W+---------------------------------------------------------+\n\r");
		}
	}
...
void do_setwage( CHAR_DATA *ch, char *argument )
{
	...
	else if ( clan->number2 == ch->name && clan->number1 == victim->name )
	{
		send_to_char( "They are more powerful than you.\n\r", ch);
		return;
	}
	else if ( clan->number3 == ch->name && clan->number2 == victim->name )
	{
		send_to_char( "They are more powerful than you.\n\r", ch);
		return;
	}
	else if ( clan->number4 == ch->name && clan->number3 == victim->name )
	{
		send_to_char( "They are more powerful than you.\n\r", ch);
		return;
	}
	else if ( clan->number5 == ch->name && clan->number4 == victim->name )
	{
		send_to_char( "They are more powerful than you.\n\r", ch);
		return;
	}
	else if ( clan->number6 == ch->name && clan->number5 == victim->name )
	{
		send_to_char( "They are more powerful than you.\n\r", ch);
		return;
	}
	else if ( clan->number7 == ch->name && clan->number6 == victim->name )
	{
		send_to_char( "They are more powerful than you.\n\r", ch);
		return;
	}
...
// TODO
void do_interclan(CHAR_DATA *ch, char *argument)
{
	/*if (NOT_AUTHED(ch))
	{
		send_to_char("Huh?\n\r", ch);
	...
	if ( IS_NPC( ch ) || !ch->pcdata->clan || !is_name("interclan", ch->pcdata->bestowments))
	{
		send_to_char( "Huh?\n\r", ch );
		return;
	}

	ch_printf( ch, "&z[&wOutgoing Message to %s&z]\n\r", clanname);
