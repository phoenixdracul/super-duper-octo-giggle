 ...
 * 						  		Arena Module					    		*
 * 																			*
 *  Allows players to challenge each other to harmless battles in an arena.	*
 *  Other players can bet on the outcome of the fight and a leaderboard		*
 *  keeps track of arena champions.											*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
 *																			*
...
/* Called from do_bet (Split with roulette) */
void do_arena_bet(CHAR_DATA *ch, char *argument)
 {
...
void do_ahall(CHAR_DATA *ch, char *argument)
{
  ...
  if (!fame_list)
  {
     ...
     sprintf(buf2,  "&B|---------------------------------------|\r\n");
     strcat(buf2, "|    &WPast Winners of The ");
     strcat(buf2, sysdata.mud_acronym);  
     strcat(buf2, " Arena&B     |\r\n");
     strcat(buf2, "|---------------------------------------|\r\n\r\n"); 
...
void do_challenge(CHAR_DATA *ch, char *argument)
{
 ...
 if(IS_SET( victim->deaf, CHANNEL_SPORTS))
 {
   send_to_char("&WThey have their sports channel turned off.\n\r", ch);
   return;
 }
