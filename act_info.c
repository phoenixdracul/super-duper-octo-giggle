 ...
 * 							 Player Info Actions	 						*
 * 																			*
 * 	Contains functions for player info commands e.g. Score, level, eq.		*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
 *																			*
...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
	...
	if ( IS_OBJ_STAT(obj, ITEM_GLOW)      )   strcat( buf, "&G&W(&OG&Yl&wo&Ww&wi&Yn&Og&G&W) "   );
	if ( IS_OBJ_STAT(obj, ITEM_HUM)       )   strcat( buf, "&G&W(&gH&Gu&wm&Wm&wi&Gn&gg&G&W) "   );
	...
	{
		if( xIS_SET( ch->act, PLR_ROOMVNUM ) && obj->pIndexData && obj->pIndexData->vnum)
		{
			/* Added by Boran. */
			strcat(buf, " &B{");
			char integer_string[32];
			integer_string[0] = '\0';
			sprintf(integer_string, "&c%d", obj->pIndexData->vnum);
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	if ( IS_EVIL(victim)
			&&   IS_AFFECTED(ch, AFF_DETECT_EVIL) && !(IS_IMMORTAL(victim) && IS_IMMORTAL(ch)) ) strcat( buf, "&R(Red Aura)&w "   );
	if ( ( victim->mana > 10 )
	...
	if ( victim && victim->position == victim->defposition && victim->long_descr[0] != '\0' )
	{
		...
		{
			strcat( buf, victim->long_descr );
			if ( IS_NPC(victim) && victim->pIndexData && IS_IMMORTAL(ch) && xIS_SET(ch->act, PLR_ROOMVNUM))
			{
				/* Added by Boran. */
				// Have to remove the newline char before appending the vnum...
				buf[strlen(victim->long_descr) - 1] = '\0';
				strcat(buf, " &B{");
				char integer_string[32];
				integer_string[0] = '\0';
				sprintf(integer_string, "&c%d", victim->pIndexData->vnum);
				strcat(buf, integer_string );
				strcat(buf, "&B}\n");
			}
   ...
   Info rewrite for sleeping/resting/standing/sitting on Objects -- Xerves */
	case POS_SLEEPING:
		...
		else
		{
			if (ch->position == POS_SITTING
				...
				strcat( buf, " is sleeping nearby.&G" );

			else
				strcat( buf, " is deep in slumber here.&G" );
		}
		break;
	case POS_RESTING:
		if (victim->on != NULL)
		...
		else
		{
			if (ch->position == POS_RESTING)
				strcat ( buf, " is sprawled out alongside you.&G" );
			else
				if (ch->position == POS_MOUNTED)
					strcat ( buf, " is sprawled out at the foot of your mount.&G" );
				else
					strcat (buf, " is sprawled out here.&G" );
		}
		break;
	case POS_SITTING:
		if (victim->on != NULL)
	...
	case POS_STANDING:
		if (victim->on != NULL)
		{
			...
			{   sprintf( colorbuf, "&w&W%s", color_str(AT_IMMORT, ch) );
			strcat( buf, colorbuf);
			strcat( buf, " is here before you.&G" );
			}
			else
				if ( ( victim->in_room->sector_type == SECT_UNDERWATER )
						&& !IS_AFFECTED(victim, AFF_AQUA_BREATH) && !IS_NPC(victim) )
					strcat( buf, " is drowning here.&G" );
				else
					if ( victim->in_room->sector_type == SECT_UNDERWATER )
						strcat( buf, " is here in the water.&G" );
					else
						if ( ( victim->in_room->sector_type == SECT_OCEANFLOOR )
							...
							if ( victim->in_room->sector_type == SECT_OCEANFLOOR )
								strcat( buf, " is standing here in the water.&G" );
							else
								if ( IS_AFFECTED(victim, AFF_FLOATING)
										|| IS_AFFECTED(victim, AFF_FLYING) )
									strcat( buf, " is hovering here.&G" );
								else
									strcat( buf, " is standing here.&G" );
		break;
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	if(victim->pIndexData && victim->pIndexData->pShop
			&& (victim->pIndexData->pShop->clanshop == TRUE
					...
					&& str_cmp(victim->pIndexData->pShop->owner, ""))
	{
		char owner_str[MAX_STRING_LENGTH];
		sprintf(owner_str, "&w$N works for &W%s&w.", victim->pIndexData->pShop->owner);
		act( AT_PLAIN, owner_str, ch, NULL, victim, TO_CHAR);
	}
	show_condition( ch, victim );

	found = FALSE;
        if( ( obj = get_eq_char( victim, WEAR_CLOAK ) ) == NULL || !can_see_obj( ch, obj ) || obj->value[4] == 0 )
        {
    ...
    else
    {
        ...
        found = TRUE;
        send_to_char( where_name[WEAR_CLOAK], ch );
        send_to_char( format_obj_to_char( obj, ch, TRUE ), ch );
        send_to_char( "\n\r", ch );

        for ( iWear = 0; iWear < MAX_WEAR; iWear++ )
        {
            if ( iWear != WEAR_WIELD && iWear != WEAR_DUAL_WIELD && iWear != WEAR_SHIELD && iWear != WEAR_HEAD && iWear != WEAR_HOLD && iWear != WEAR_EARS && iWear != WEAR_EYES && iWear!= WEAR_FEET && iWear != WEAR_BACK ) 
                 continue;
...
void show_ships_to_char( SHIP_DATA *ship, CHAR_DATA *ch )
{
	...
	{
		ch_printf( ch , "&W%s%-*s     ", color_str(AT_SHIP, ch), 35 + strlen(rship->name) - strlen(strip_color(rship->name)), rship->name);
		if ( ( nship = rship->next_in_room ) !=NULL )
		{
			ch_printf( ch , "&W%s%-35s", color_str(AT_SHIP, ch), nship->name);
...
void do_look( CHAR_DATA *ch, char *argument )
{
	...
	if ( arg1[0] == '\0' || !str_cmp( arg1, "auto" ) )
	{
		...
		ch_printf(ch, "&C-=&b[ &G&W%s%s &G&b]&C=-&C&w ", color_str( AT_RMNAME, ch), ch->in_room->name);
		...
		if ( str_cmp( arg1, "auto" ) )
			if (   (ship = ship_from_cockpit(ch->in_room->vnum))  != NULL )
			{
				...
				{
					ch_printf( ch, "&YMillions of stars flashing by in hyperspace.&W\n\r");
				}
...
void show_condition( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if (( IS_NPC ( victim ) && xIS_SET( victim->act , ACT_DROID )) || IS_DROID(victim))
	{
		...
		else if ( percent >=  60 ) strcat( buf, " has a couple of holes in its plating.\n\r"    );
...
void do_examine( CHAR_DATA *ch, char *argument )
{
	...
	if ( ( obj = get_obj_here( ch, arg ) ) != NULL )
	{
		...
		{
			if ( board->num_posts )
				ch_printf( ch, "There are %d notes posted here.  Type 'note list' to list them.\n\r", board->num_posts );
			else
...
void do_exits( CHAR_DATA *ch, char *argument )
{
	...
	scraft = IS_SET(ch->in_room->room_flags, ROOM_SPACECRAFT) ? TRUE : FALSE;
	if ( IS_SET(ch->pcdata->flags, PCFLAG_NOSHIPDIR) )
		scraft = FALSE;
	...
	for ( pexit = ch->in_room->first_exit; pexit; pexit = pexit->next )
	{
				...
				&&  !IS_SET(pexit->exit_info, EX_HIDDEN) )
		{
			...
			if ( !fAuto )
			{
				...
				{
					if (!scraft)
						sprintf( buf + strlen(buf), "%s%-5s - (closed)&w&W\n\r",
							color_str(AT_EXITS, ch), capitalize( dir_name[pexit->vdir] ) );
					else
						sprintf( buf + strlen(buf), "%s%19s - (closed)&w&W\n\r",
							color_str(AT_EXITS, ch), capitalize(ship_dir_name[pexit->vdir]));
				}
				else if ( IS_SET(pexit->exit_info, EX_WINDOW) )
				{
					if (!scraft)
						sprintf( buf + strlen(buf), "%s%-5s - (window)&w&W\n\r",
							color_str(AT_EXITS, ch), capitalize( dir_name[pexit->vdir] ) );
					else
						sprintf( buf + strlen(buf), "%s%19s - (window)&w&W\n\r",
                                                        color_str(AT_EXITS, ch), capitalize(ship_dir_name[pexit->vdir]));
				}
				...
				{
					if (!scraft)
						sprintf( buf + strlen(buf), "%s%-5s - %s%s&c %s&w&W\n\r",
							color_str(AT_EXITS, ch), capitalize( dir_name[pexit->vdir] ),color_str( AT_RMNAME, ch),
							room_is_dark( pexit->to_room )
							?  "Too dark to tell"
									: pexit->to_room->name ,
							xIS_SET(ch->act, PLR_ROOMVNUM) ? itoa(pexit->to_room->vnum) : "");
					else
						sprintf( buf + strlen(buf), "%s%-19s - %s%s&c %s&w&W\n\r",
                                                        color_str(AT_EXITS, ch), capitalize( ship_dir_name[pexit->vdir] ),color_str( AT_RMNAME, ch),
                                                        room_is_dark( pexit->to_room )
                                                        ?  "Too dark to tell"
                                                                        : pexit->to_room->name ,
                                                        xIS_SET(ch->act, PLR_ROOMVNUM) ? itoa(pexit->to_room->vnum) : "");
			...
			else
			{
				sprintf( buf + strlen(buf), " %s&w&W",
						capitalize( !scraft ? dir_name[pexit->vdir] : ship_dir_name[pexit->vdir] ) );
			}
		}
	}
   ...
   No need.
	...
	&&   wch->pcdata->homepage[0] != '\0' )
	  sprintf( char_name, %s,"<A HREF=\"%s\">%s</A>",
		 ...
		 */
		strcpy( char_name, "" );
		if((wch->rank == NULL || !str_cmp(wch->rank, "&R[&wnull&R]&w")) && !IS_IMMORTAL(wch))
			wch->rank = str_dup("   ");
                if( IS_IMMORTAL( wch ) )
     		    sprintf( race_text, "&G&w%-20s&G&c &G&w", wch->rank);
                else
                    sprintf( race_text, "&G&c &G&w" );
		race = race_text;
		...
		if ( !wch->rank || (wch->rank[0] == '\0')  ) 	//If the imm has set thier rank show it instead --Odis ->KeB
		{
			...
			case MAX_LEVEL -  4: race = "&G&z(&G&wImmortal     &G&z)&G&w";	break;
			case MAX_LEVEL -  5: race = "&G&z(&G&wTrainee      &G&z)&G&w";	break;
			}
		}


		if ( ch && !nifty_is_name(wch->name, remand(wch->name)) && ch->top_level > wch->top_level )
	  ...
	  race = wch->rank;
...

		{
			...
			if ( !str_cmp( wch->name, pclan->number2 ) )
				strcat( clan_name, "Second, " );
                        if ( !str_cmp( wch->name, pclan->number3 ) )
                                strcat( clan_name, "Third, " );
                        if ( !str_cmp( wch->name, pclan->number4 ) )
                                strcat( clan_name, "Fourth, " );
                        if ( !str_cmp( wch->name, pclan->number5 ) )
                                strcat( clan_name, "Fifth, " );
                        if ( !str_cmp( wch->name, pclan->number6 ) )
                                strcat( clan_name, "Sixth, " );
                        if ( !str_cmp( wch->name, pclan->number7 ) )
                                strcat( clan_name, "Seventh, " );

			strcat( clan_name, pclan->name );
			strcat( clan_name, "&R]&z" );
		}
		else
			clan_name[0] = '\0';
		...
		if ( xIS_SET(wch->act, PLR_WIZINVIS) )
			...
			invis_str[0] = '\0';
		char afk_msg_buf[MAX_STRING_LENGTH];
		afk_msg_buf[0] = '\0';
		if(xIS_SET(wch->act, PLR_AFK) && wch->afk_msg && wch->afk_msg[0] != '\0')
			sprintf(afk_msg_buf, "&R(&w%s&R) ", wch->afk_msg);
			
		sprintf( buf, "%s   %s%s%s%s %s%s\n%s",
				race,
				invis_str,
				xIS_SET(wch->act, PLR_AFK) ? "&R[&wAFK&R]&w" : "",
						char_name,
						wch->pcdata->title,
						(xIS_SET(wch->act, PLR_AFK) && afk_msg_buf[0] != '\0') ? afk_msg_buf : "",
						clan_name,
						ch ? "\r" : "");
	...
	if ( first_newbie )
	{
		if ( !ch )
		{
			sprintf( mudnamebuffer, "&G===&B[ &P%s &B]&G===&B[ &WMortals &B]&G=====================================================&B[]&W\n\n", sysdata.mud_acronym );
	...
	if ( !ch )
	{
		//	fprintf( whoout, "%s", htmlcolor("\n\r&&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&W[ &R%d &Gvisible players &W]&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&B-&b=&w\n"),
	...
	fclose( whoout );
	ch_printf( ch, "\n\r&z-=-=-=-=-=-=-=-=-=-=-=-=-=-=-&w[ &C%d &cvisible players &w]&z-=-=-=-=-=-=-=-=-=-=-=-=-=&W\n\r",
			sMatch, sMatch == 1 ? "" : "s", nMatch, nMatch == 1 ? "" : "s" );
	return;
}
...
void do_practice( CHAR_DATA *ch, char *argument )
{
	...
	if(argument[0] != '\0')
	{
		...
		strcpy(argRemainder, one_argument( argument, arg1 ));
		arg1[0] = UPPER(arg1[0]);
		int ind;
		for(ind = 0; ind < MAX_ABILITY; ++ind )
		{
			if((strcasecmp(arg1, ability_name[ind]) == 0 && argRemainder[0] == '\0') || strcasecmp(argument, ability_name[ind]) == 0)
			{
				classFound = TRUE;
				abilityIndex = ind;
				if( strcasecmp(ability_name[ind], "force") == 0)
				{
					send_to_char("&RYou can't view force skills with prac. Type &Wfinfo&R.&w\n\r", ch);
					return;
				}
				break;
			}
	...
	if ( argument[0] == '\0' || classFound == TRUE)
	{
		int	col;
		sh_int	lasttype, cnt;
		int	color = 0;
		...
		for ( sn = 0; sn < top_sn; sn++ )
		{
			...
			if(classFound == TRUE && skill_table[sn]->guild != abilityIndex)
				continue;
			...
			if ( skill_table[sn]->type != lasttype )
			{
				...
				if (!str_cmp(skill_tname[skill_table[sn]->type], "weapon"))
					pager_printf( ch,"&c-----------------------------------&B[&W%ss&B]&c------------------------------------\n\r", skill_tname[skill_table[sn]->type]);
				else
					pager_printf( ch,"&c------------------------------------&B[&W%s&B]&c-------------------------------------\n\r",
...
void do_channels( CHAR_DATA *ch, char *argument )
{
	...
	if ( arg[0] == '\0' )
	{
		...
		{
			send_to_char( !IS_SET(ch->deaf2, CHANNEL_CODENET)
					? " +CODENET"
							: " -codenet",
							  ch );
		}
		...
		{
			send_to_char( !IS_SET(ch->deaf2, CHANNEL_BUILDNET)
					? " +BUILDNET"
							: " -buildnet",
							  ch );
		}
	...
	else
	{
		...
		else if ( !str_cmp( arg+1, "codenet") && get_trust(ch) >= LEVEL_SUB_IMPLEM )
		{
			bit = CHANNEL_CODENET;
			deaf_set = 2;
		}
		else if ( !str_cmp( arg+1, "buildnet") && get_trust(ch) >= LEVEL_LESSER )
		{
			bit = CHANNEL_BUILDNET;
			deaf_set = 2;
		}
		...
		} else if (fClear)
		{
			if(deaf_set == 1)
				...
				REMOVE_BIT (ch->deaf2, bit);
		} else
		{
			if(deaf_set == 1)
				SET_BIT (ch->deaf, bit);
			else if(deaf_set == 2)
				SET_BIT (ch->deaf2, bit);
		}
...
void do_config( CHAR_DATA *ch, char *argument )
{
	...
	if ( arg[0] == '\0' )
	{
		...
		send_to_char( IS_SET(ch->pcdata->flags, PCFLAG_AUTODRAW)
				? "[+AUTODRAW ] You automatically draw weapons when attacked.\n\r"
						: "[-autodraw ] You don't automatically draw weapons when attacked.\n\r"
						, ch);
		...
		send_to_char(  IS_SET(ch->pcdata->flags, PCFLAG_NOINTRO)
                ...
                send_to_char(  IS_SET(ch->pcdata->flags, PCFLAG_NOPROX)
                                ? "[+NOPROX   ] You don't see the proximity warning spam.\n\r"
                                                : "[-noprox   ] You see the proximity warning spam.\n\r"
                                                , ch );

		send_to_char(  IS_SET(ch->pcdata->flags, PCFLAG_NOSHIPDIR)
				? "[+NOSHIPDIR] You see exits only as north, east, south, west, etc. on ships.\n\r"
						: "[-noshipdir] You see proper exit names on ships.\n\r"
						, ch );
		...
		send_to_char(  IS_SET(ch->pcdata->flags, PCFLAG_256COL)
				? "[+256COL   ] You receive 256 color ANSI sequences.\n\r"
						: "[-256col   ] You don't receive 256 color ANSI sequences.\n\r"
						, ch );

		send_to_char( IS_SET(ch->pcdata->flags, PCFLAG_NOIR)
				? "[+NOIR     ] Noir color theme enabled. Overrides other color stuff.\n\r"
						: "[-noir     ] Noir color theme disabled.\n\r"
						, ch );
		...
		if ( IS_IMMORTAL( ch ) )
			send_to_char(  xIS_SET(ch->act, PLR_ROOMVNUM)
					? "[+VNUM     ] You can see the VNUM of the current room, mobs and objects.\n\r"
							: "[-vnum     ] You do not see the VNUM of the current room, mobs and objects.\n\r"
							, ch );
	...
	else
	{
		...
		else
		{
			...
			else if ( !str_prefix( arg+1, "autodraw" ) ) bit = PCFLAG_AUTODRAW;
			else if ( !str_prefix( arg+1, "gag"      ) ) bit = PCFLAG_GAG;
			else if ( !str_prefix( arg+1, "compass" ) ) bit = PCFLAG_MAP;
                        else if( !str_prefix( arg+1, "noprox" ) ) bit = PCFLAG_NOPROX;
			else if ( !str_prefix( arg+1, "noshipdir") ) bit = PCFLAG_NOSHIPDIR;
			else if ( !str_prefix( arg+1, "256col"   ) ) bit = PCFLAG_256COL;
			else if ( !str_prefix( arg+1, "noir"	 ) ) bit = PCFLAG_NOIR;
...
void do_afk( CHAR_DATA *ch, char *argument )
{
	...
	{
		/* Added by Boran - AFK message */
		if(argument && argument[0] != '\0')
		{
			if(ch->afk_msg)
				DISPOSE(ch->afk_msg);
			ch->afk_msg = str_dup(argument);
		}
		else if(ch->afk_msg)
			DISPOSE(ch->afk_msg);
...
void do_oldslist( CHAR_DATA *ch, char *argument )
{
	int sn, i, lFound;
...
void do_slist( CHAR_DATA *ch, char *argument )
{
	...
	// Check if player has entered an ability as an arg.
	if(arg1[0] != '\0')
	{
		// Check if ability exists
		int abilityExists;
		...
		for(ind = 0; ind < MAX_ABILITY; ++ind )
		{
			if(strcasecmp(arg1, ability_name[ind]) == 0 || strcasecmp(argument, ability_name[ind]) == 0)	// Player doesn't have to use quotes
			{
				if(strcasecmp(argument, ability_name[ind]) == 0)		// Player doesn't have to use quotes
				{
					strcpy(arg1, argument);
				}
				if(ind == FORCE_ABILITY)
				{
					send_to_char("&RYou cannot see force abilities with slist. Type &Wfinfo&R.&w\n\r", ch);
					return;
				}
				else
				{
		...
		if(!abilityExists)
		{
			send_to_char("&RSyntax: 'slist' &Wor&R 'slist &z<&Rclass&z>&R'&w\n\r", ch);	// Player entered invalid ability
			return;
		}
	}
	lowlev = 1;
	...
	for ( ability = 0 ; ability < MAX_ABILITY ; ability++ )
	{
		...
		if (strcasecmp(arg1, ability_name[ability]) != 0
				&& arg1[0] != '\0')
			continue;
		...
		for (i=lowlev; i <= hilev; i++)
		{
			...
			for ( sn = 0; sn < top_sn; sn++ )
			{
				...
				if(i == skill_table[sn]->min_level)
				{
					pager_printf(ch, "&c(&W%2d&c)%s %s%-18.18s&W  ",
							i, skill_table[sn]->alignment >= 1001 ? "&B" :
									skill_table[sn]->alignment <= -1001 ? "&R" :
											"&w", color_str(AT_SLIST, ch), skill_table[sn]->name );
...
void do_whois( CHAR_DATA *ch, char *argument)
{
	...
	if(!IS_IMMORTAL(ch) && xIS_SET(victim->act, PLR_WHOINVIS) )
	{
		send_to_char("No such player online. Remember, you must input the whole name of the player.\n\r", ch);
		return;
	}
	...
	if(IS_IMMORTAL(ch))
	{
		...
		if (get_trust(ch) >= LEVEL_GOD && get_trust(ch) >= get_trust( victim ) && victim->pcdata )
		{
			sprintf (buf2, "Email: %s\n\r" , victim->pcdata->email );
                        send_to_char(buf2, ch);
			sprintf (buf2, "Mentor/Mentee: %s\n\r" , victim->pcdata->mentor );
			send_to_char(buf2, ch);
		}
	}
}
...
void do_whoinvis( CHAR_DATA *ch, char *argument )
{
	...
	if xIS_SET(ch->act, PLR_WHOINVIS)
    		 {
		xREMOVE_BIT(ch->act, PLR_WHOINVIS);
		send_to_char( "You now show up on the who list.\n\r", ch );
    		 }
	else
	{
		xSET_BIT(ch->act, PLR_WHOINVIS);
	...
	for ( pexit = ch->in_room->first_exit; pexit; pexit = pexit->next )
	{
				...
				&& !IS_SET(pexit->exit_info, EX_HIDDEN) )
		{
			...
			if ( !fAuto )
			{
				...
				{
					sprintf( buf + strlen(buf), "&G&W%-9s &R- &G&W%s%s&c %s&w&W",
					...
					sprintf( buf + strlen(buf), "&W%-9s &R- &W%s%s&c %s&w&W",
