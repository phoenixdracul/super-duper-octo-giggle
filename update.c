...
void 	claim_time args( ( void ) );
void    lottery_draw( void );
void	update_taxes	args( ( void ) );
void	update_copytimer args( (void)  );
...
/*		skill = URANGE(1, (ch->skill_level[ability] + 10) / 20, 4);
		if (ch->skill_level[ability] > 15)
			...
			skill += 1;*/
		skill = ((ch->skill_level[ability])+((ch->perm_int)*4))/10;
...
/* Disabled to keep people from dying, per request. -- Kasji
	if ( get_age( ch ) > 800 )
	{
...
void mobile_update( void )
{
	...
	for ( ch = last_char; ch; ch = gch_prev )
	{
		...
		if ( !xIS_SET(ch->act, ACT_RUNNING)
				...
				&& (( !xIS_SET(ch->act, ACT_STAY_AREA) && pexit->to_room->area)
...
/*
 * Updates the copyover timer, if it is active.
 */
void update_copytimer( void )
{
	if(copyover_set && --copytimer_seconds <= 0)
	{
		/* Run the copyover */
		copytimer_seconds = 1000;
		do_copyover(copyover_ch, "");
	}
	return;
}
/*
...
{   
	...
	if ( ch->variables )
	{
		...
		if ( !char_died(ch) )
		{
			...
			if ( !IS_NPC(ch) && !IS_IMMORTAL(ch) && ch->in_room->area && ch->in_room->area->planet )
			{
				if( ch->in_room->area->planet->atmosphere_type == ATMO_GASEOUS
					// TODO: && !IS_AFFECTED(ch, AFF_GAS_BREATH) 
					)
				{
					send_to_char( "&rYou choke as you breath in the toxic astmosphere of this planet.&w\n\r", ch );
					ch->hit -= number_range(1, 5);
					update_pos(ch);
				}
				if ( // TODO: !IS_AFFECTED(ch, AFF_HEAT_RESIST) && 
					ch->in_room->area->planet->atmosphere_type == ATMO_HEAT )
				{
					send_to_char( "&rThe extreme heat of this planet takes its toll on your body.&w\n\r", ch );
					ch->hit -= number_range( 1, 5 );
					update_pos(ch);
				}
				if ( // TODO: !IS_AFFECTED(ch, AFF_COLD_RESIST) && 
					ch->in_room->area->planet->atmosphere_type == ATMO_BLIZZARD && !IS_SET(ch->in_room->room_flags, ROOM_INDOORS) )
				{
					send_to_char( "&rYour body shivers from the extreme cold of this desolate planet.&w\n\r", ch );
					ch->hit -= number_range( 1, 5 );
					update_pos(ch);
				}
			}
			...
			{
				char buf[MAX_STRING_LENGTH];
				amount = ch->pcdata->bank * sysdata.bank_interest;
...
void claim_time(  )
{
   ...
   if( lottery.claim_time > 0 && lottery.lottery_winner > 0 )
   {
      lottery.claim_time--;
      save_lottery( lottery );
   }
   else if( lottery.claim_time == 0 )
   {
      sprintf( buf,
               "&RL&ro&zt&wt&ze&rr&Ry&B: &RNo one has claimed their prize, winnings carried over to next lottery.\n\r" );
      lottochan( buf );
      lottery.last_winner = str_dup( "no one" );
      ...
      new_pot = ( lottery.lottery_pot + 1000000 );

      lottery.lottery_pot = new_pot;
      save_lottery( lottery );
   }
   return;
}
...
{
   char buf[MAX_STRING_LENGTH];
   int lottery_winner = 0;

   if( lottery.next_draw == 0 && lottery.lottery_winner == 0 )
   {
      lottery_winner = number_range( 1, 100 );
      lottery.lottery_winner = lottery_winner;
      sprintf( buf, "&RL&ro&zt&wt&ze&rr&Ry&B: &CThe winning number for the lottery is &w%d&C.\n\r", lottery_winner );
      lottery.claim_time = 1440; /* 24 Hours */
      lottery.next_draw = -1;
      save_lottery( lottery );
      lottochan( buf );
   }
...
void lottery_update(  )
{
   ...
   if( lottery.lottery_next > 0 )
   {
      lottery.lottery_next--;
      save_lottery( lottery );
      return;
   }

   if( lottery.lottery_next == 0 && lottery.lottery_winner == 0 )
   {

      if( lottery.lottery_pot >= 50000000 )
      {
         lottery.lottery_pot = 50000000;
         sprintf( buf, "Lottery has reach the maximum pot size of 25,000,000 credits!\n\r" );
         lottochan( buf );
         lottery.lottery_next = 60;
         return;
      }

      lottery_plus = ( num_descriptors * 1000 );
      if(lottery_plus < 0)
    	  lottery_plus = 0;

      lottery.lottery_pot += lottery_plus;
      sprintf( buf, "&GH&go&wur&gl&Gy &GU&gp&wda&gt&Ge&B: &Y%s &Ccredits have been added to this weeks lottery pot.\n\r",
                num_punct(lottery_plus) );
      lottochan( buf );
      lottery.lottery_next = 60;
      save_lottery( lottery );
   }
...
void update_handler( void )
{
	...
	static  int 	pulse_bet = PULSE_BET;
	static 	int 	pulse_lottery;
	...
	static  int		copyover_pulse = PULSE_PER_SECOND;
	struct timeval stime;
	...
	if (copyover_set && --copyover_pulse <= 0)
	{
		copyover_pulse = PULSE_PER_SECOND;
		update_copytimer();
	}
...
void auction_update (void)
{
	...
	case 3 : /* SOLD! */
		...
		{
			sprintf (buf, "&W%s&W is &wSOLD&W for &Y%d&W credits.",
					auction->item->short_descr,
					auction->bet);
			talk_auction(buf);
