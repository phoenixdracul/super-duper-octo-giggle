 ...
 * 						     Interpreter Module		 					    *
 * 																			*
 *  Handles input from the client and decides what to do with it.			*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
 *																			*
...
void interpret( CHAR_DATA *ch, char *argument )
{
	...
	if ( !cmd )
	{
			...
			if ( !str_prefix( command, cmd->name ))
			{
				if((cmd->level <= trust
						||  (!IS_NPC(ch) && ch->pcdata->bestowments && ch->pcdata->bestowments[0] != '\0'
								...
								&&    cmd->level <= (trust+5)) ))
				{
					/* Manually put buildnet channel with higher priority than buildwalk - Boran */
					if(!str_cmp(cmd->name, "buildwalk") && !str_prefix( command, "buildn"))
					{
						char tempcom[MAX_STRING_LENGTH];
						strcpy(tempcom, "buildnet");
						strcat(tempcom, " ");
						strcat(tempcom, argument);
						interpret(ch, tempcom);
						return;
					}
					found = TRUE;
					break;
		...
		/*
		 * Turn off afk bit when any command performed. EDIT: Changed by Boran.
		 */
		if ( !IS_NPC(ch) && xIS_SET ( ch->act, PLR_AFK)  && (str_cmp(command, "AFK")))
		{
			/* Changed by Boran. */
			/* Allows certain commands to be run, otherwise tells player that he's afk. */
			if((cmd && (!cmd->ooc && str_cmp(cmd->name, "osay") && str_cmp(cmd->name, "ooc") && str_cmp(cmd->name, "score")
					&& str_cmp(cmd->name, "slist") && str_cmp(cmd->name, "help") && str_cmp(cmd->name, "mstat")
					&& str_cmp(cmd->name, "tell") && str_cmp(cmd->name, "gboard") && str_cmp(cmd->name, "gnote")
					&& str_cmp(cmd->name, "reply") && str_cmp(cmd->name, "immtalk") && str_cmp(cmd->name, "who")
					&& str_cmp(cmd->name, "quit") && str_cmp(cmd->name, "changes") && str_cmp(cmd->name, "addchange")
					&& str_cmp(cmd->name, "invisible") && str_cmp(cmd->name, "admin") && str_cmp(cmd->name, "idealog")
					&& str_cmp(cmd->name, "users") && str_cmp(cmd->name, "level") && str_cmp(cmd->name, "project"))
					&& str_cmp(cmd->name, "wizhelp") && str_cmp(cmd->name, "lastplayed")
					&& str_cmp(cmd->name, "lottery")) || no_trust)
			{
				send_to_char("&WYou are AFK. Type AFK to return.&w\n\r", ch);
				return;
			}
			// else allow the command to proceed.
		}
	}
	...
	else if ( ( !IS_NPC(ch) && xIS_SET(ch->act, PLR_LOG) )
			||   fLogAll
			||	 loglvl == LOG_BUILD
			||   loglvl == LOG_HIGH
			||   loglvl == LOG_ALWAYS )
	 ...
	 */
	if ( ( !IS_NPC(ch) && xIS_SET(ch->act, PLR_SLOG) ))
	{
		/* Added by Narn to show who is switched into a mob that executes
           a logged command.  Check for descriptor in case force is used. */
		if ( ch->desc && ch->desc->original )
...
bool check_social( CHAR_DATA *ch, char *command, char *argument )
{
	...
	if ( arg[0] == '\0' )
	{
		act( color, social->others_no_arg, ch, NULL, victim, TO_ROOM    );
		act( color, social->char_no_arg,   ch, NULL, victim, TO_CHAR    );
	}
	...
	{
		act( color, social->others_auto,   ch, NULL, victim, TO_ROOM    );
		act( color, social->char_auto,     ch, NULL, victim, TO_CHAR    );
	}
	else
	{
		act( color, social->others_found,  ch, NULL, victim, TO_NOTVICT );
		act( color, social->char_found,    ch, NULL, victim, TO_CHAR    );
		act( color, social->vict_found,    ch, NULL, victim, TO_VICT    );
				...
				&&   !IS_SET( victim->pIndexData->progtypes, ACT_PROG ) )
		{
			...
			case 0:
				if ( !IS_SET(ch->in_room->room_flags, ROOM_SAFE )
					...
					if ( IS_NEUTRAL(ch) )
					{
						act( color, "$n slaps $N.",  victim, NULL, ch, TO_NOTVICT );
						act( color, "You slap $N.",  victim, NULL, ch, TO_CHAR    );
						act( color, "$n slaps you.", victim, NULL, ch, TO_VICT    );
					}
					else
					{
						act( color, "$n acts like $N doesn't even exist.",  victim, NULL, ch, TO_NOTVICT );
						act( color, "You just ignore $N.",  victim, NULL, ch, TO_CHAR    );
						act( color, "$n appears to be ignoring you.", victim, NULL, ch, TO_VICT    );
					}
				break;

			case 1: case 2: case 3: case 4:
			case 5: case 6: case 7: case 8:
				act( color, social->others_found,
						victim, NULL, ch, TO_NOTVICT );
				act( color, social->char_found,
						victim, NULL, ch, TO_CHAR    );
				act( color, social->vict_found,
