 ...
 * 							   Noteboard Module	 							*
 * 																			*
 * 	Physical note boards which reside in the game world. Usually used for	*
 *  IC notes. e.g. Clan boards, guild boards, personal mail.				*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
 *																			*
...
void do_note( CHAR_DATA *ch, char *arg_passed, BOOL IS_MAIL )
{
	...
	case SUB_WRITING_NOTE:
		...
		//		if(strlen(copy_buffer(ch)) > MAX_STRING_LENGTH)
		//		{
		//			send_to_char("&RLine too long.", ch);
		//			stop_editing(ch);
		//			return;
		//		}
