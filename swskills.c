 ...
 * 							  	  SW Skills 	 							*
 * 																			*
 * 	Most of the skills for the classes will be found here.					*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
 *																			*
...
void do_makeblade( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		/* If char is hiding, make him visible - Added by Boran */
		if ( IS_AFFECTED(ch, AFF_HIDE) )
		{
			REMOVE_BIT(ch->affected_by, AFF_HIDE);
			send_to_char( "&wYou step out of the shadows to begin your work.\n\r", ch);
			act( AT_PLAIN, "&z$n steps out of the shadows.", ch,
					NULL, argument , TO_ROOM );
		}

		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makeblade] * 100 / (ch->pcdata->learned[gsn_makeblade] + 1));
...
//		xpgain = UMIN( obj->cost*200 ,( exp_level(ch->skill_level[ENGINEERING_ABILITY]+1) - exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) );
		xpgain = UMIN( obj->cost*200 ,( exp_gain(ch->skill_level[ENGINEERING_ABILITY], gsn_makearmor) ) );
...
void do_makeblaster( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makeblaster] * 100 / (ch->pcdata->learned[gsn_makeblaster] + 1));
		if ( number_percent( ) < chance )
		{
...
//		xpgain = UMIN( obj->cost*50 ,( exp_level(ch->skill_level[ENGINEERING_ABILITY]+1) - exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) );
		xpgain = UMIN( obj->cost*50 ,( exp_gain(ch->skill_level[ENGINEERING_ABILITY], gsn_makeblaster) ) );
		gain_exp(ch, xpgain, ENGINEERING_ABILITY);
		ch_printf( ch , "You gain %d engineering experience.", xpgain );
	}
...
void do_makelightsaber( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makelightsaber] * 100 / (ch->pcdata->learned[gsn_makelightsaber] + 1));
		if ( number_percent( ) < chance )
		{
...

	{
		...
		xpgain = UMIN( obj->cost*50 , exp_gain(ch->skill_level[FORCE_ABILITY], gsn_makelightsaber));
		// Changed. -Tawnos
...
void do_makemedpac( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makemedpac] * 100 / (ch->pcdata->learned[gsn_makemedpac] + 1));
		if ( number_percent( ) < chance )
		{
    ...
    {
    	long xpgain;
//    	xpgain = UMIN( 10000 , exp_level(ch->skill_level[COMBAT_ABILITY]+1) - exp_level(ch->skill_level[COMBAT_ABILITY]));
    	xpgain = UMIN( 10000 , exp_gain(ch->skill_level[COMBAT_ABILITY], gsn_makemedpac));
    	gain_exp(ch, xpgain, COMBAT_ABILITY);
    	ch_printf( ch , "You gain %d combat experience.", xpgain );
...
void do_makespice( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_spice_refining] * 100 / (ch->pcdata->learned[gsn_spice_refining] + 1));
		if ( number_percent( ) < chance )
		{
...
//		xpgain = UMIN( obj->cost*50 ,( exp_level(ch->skill_level[ENGINEERING_ABILITY]+1) - exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) );
		xpgain = UMIN( obj->cost*50 ,( exp_gain(ch->skill_level[ENGINEERING_ABILITY], gsn_spice_refining) ) );
		gain_exp(ch, xpgain, ENGINEERING_ABILITY);
		ch_printf( ch , "You gain %d engineering experience.", xpgain );
	}
...
void do_makegrenade( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makegrenade] * 100 / (ch->pcdata->learned[gsn_makegrenade] + 1));
		if ( number_percent( ) < chance )
		{
...
//		xpgain = UMIN( obj->cost*50 ,( exp_level(ch->skill_level[ENGINEERING_ABILITY]+1) - exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) );
		xpgain = UMIN( obj->cost*50 ,( exp_gain(ch->skill_level[ENGINEERING_ABILITY], gsn_makegrenade) ) );
		gain_exp(ch, xpgain, ENGINEERING_ABILITY);
		ch_printf( ch , "You gain %d engineering experience.", xpgain );
	}
...
void do_makelandmine( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makelandmine] * 100 / (ch->pcdata->learned[gsn_makelandmine] + 1));
		if ( number_percent( ) < chance )
		{
...
//		xpgain = UMIN( obj->cost*50 ,( exp_level(ch->skill_level[ENGINEERING_ABILITY]+1) - exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) );
		xpgain = UMIN( obj->cost*50 ,( exp_gain(ch->skill_level[ENGINEERING_ABILITY], gsn_makelandmine) ) );
		gain_exp(ch, xpgain, ENGINEERING_ABILITY);
		ch_printf( ch , "You gain %d engineering experience.", xpgain );
	}
...
void do_makelight( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makelight] * 100 / (ch->pcdata->learned[gsn_makelight] + 1));
		if ( number_percent( ) < chance )
		{
...
//		xpgain = UMIN( obj->cost*100 ,( exp_level(ch->skill_level[ENGINEERING_ABILITY]+1) - exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) );
		xpgain = UMIN( obj->cost*100 ,( exp_gain(ch->skill_level[ENGINEERING_ABILITY], gsn_makelight) ) );
		gain_exp(ch, xpgain, ENGINEERING_ABILITY);
		ch_printf( ch , "You gain %d engineering experience.", xpgain );
	}
...
void do_makejewelry( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makejewelry] * 100 / (ch->pcdata->learned[gsn_makejewelry] + 1));
		if ( number_percent( ) < chance )
		{
...
//		xpgain = UMIN( obj->cost*100 ,( exp_level(ch->skill_level[ENGINEERING_ABILITY]+1) - exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) );
		xpgain = UMIN( obj->cost*100 ,( exp_gain(ch->skill_level[ENGINEERING_ABILITY], gsn_makejewelry) ) );
		gain_exp(ch, xpgain, ENGINEERING_ABILITY);
		ch_printf( ch , "You gain %d engineering experience.", xpgain );
	}
...
void do_makearmor( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makearmor] * 110 / (ch->pcdata->learned[gsn_makearmor] + 2));
...
//		xpgain = UMIN( obj->cost*100 ,( exp_level(ch->skill_level[ENGINEERING_ABILITY]+1) - exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) );
		xpgain = UMIN( obj->cost*100 ,( exp_gain(ch->skill_level[ENGINEERING_ABILITY], gsn_makearmor) ) );
		gain_exp(ch, xpgain, ENGINEERING_ABILITY);
...
void do_makecomlink( CHAR_DATA *ch, char *argument)
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makecomlink] * 100 / (ch->pcdata->learned[gsn_makecomlink] + 1));
		if ( number_percent( ) < chance )
		{
...
//		xpgain = UMIN( obj->cost*100 ,( exp_level(ch->skill_level[ENGINEERING_ABILITY]+1) - exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) );
		xpgain = UMIN( obj->cost*100 ,( exp_gain(ch->skill_level[ENGINEERING_ABILITY], gsn_makecomlink) ) );
		gain_exp(ch, xpgain, ENGINEERING_ABILITY);
		ch_printf( ch , "You gain %d engineering experience.", xpgain );
	}
...
void do_makeshield( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makeshield] * 100 / (ch->pcdata->learned[gsn_makeshield] + 1));
		if ( number_percent( ) < chance )
		{
...
//	xpgain = UMIN( obj->cost*5 ,( exp_level(ch->skill_level[ENGINEERING_ABILITY]+1) - exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) );
	xpgain = UMIN( obj->cost*5 ,( exp_gain(ch->skill_level[ENGINEERING_ABILITY], gsn_makeshield) ) );
	gain_exp(ch, xpgain, ENGINEERING_ABILITY);
	ch_printf( ch , "You gain %d engineering experience.", xpgain );
...
void do_makecontainer( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makecontainer] * 100 / (ch->pcdata->learned[gsn_makecontainer] + 1));
		if ( number_percent( ) < chance )
		{
...
//		xpgain = UMIN( obj->cost*100 ,( exp_level(ch->skill_level[ENGINEERING_ABILITY]+1) - exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) );
		xpgain = UMIN( obj->cost*100 ,( exp_gain(ch->skill_level[ENGINEERING_ABILITY], gsn_makecontainer) ) );
		gain_exp(ch, xpgain, ENGINEERING_ABILITY);
		ch_printf( ch , "You gain %d engineering experience.\n\r", xpgain );
	}
...
void do_makeholster( CHAR_DATA *ch, char *argument )
{
	char arg[MAX_INPUT_LENGTH];
	char buf[MAX_STRING_LENGTH];
	int level, chance;
...
//	int value;
	...
	default:
		...
		if ( arg[0] == '\0' )
		{
			send_to_char( "&RUsage: Makeholster <name>\n\r&w", ch);
			return;
		}
		...
		if( !checkdura )
		{
			send_to_char("&RYou'll need some duraplast to make it out of.\n\r", ch);
			return;
		}
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makeholster] * 100 / (ch->pcdata->learned[gsn_makeholster] + 1));
		if ( number_percent( ) < chance )
		{
			send_to_char( "&GYou begin working on your holster.\n\r", ch);
			act( AT_PLAIN, "$n takes $s sewing kit and some material and begins to work.", ch,
					NULL, argument , TO_ROOM );
			add_timer ( ch , TIMER_DO_FUN , 10 , do_makeholster , 1 );
			ch->dest_buf = str_dup(arg);
			return;
		}
		send_to_char("&RYou can't figure out what to do.\n\r",ch);
		learn_from_failure( ch, gsn_makeholster );
		return;
	...
	level = IS_NPC(ch) ? ch->top_level : (int) (ch->pcdata->learned[gsn_makeholster]);
	...
	if ( number_percent( ) > chance  || ( !checkfab ) || (!checktool) || ( !checksew ) || (!checkdura))
	{
		...
		send_to_char( "&Rprocess and your holster falls to pieces in front of you...\n\r", ch);
		learn_from_failure( ch, gsn_makeholster );
		return;
	}
	...
	send_to_char( "&GYou finish your work and hold up your newly crafted holster.&w\n\r", ch);
	act( AT_PLAIN, "$n finishes crafting a holster.", ch,
			NULL, argument , TO_ROOM );

	{
		long xpgain;

//		xpgain = UMIN( obj->cost*300 ,( exp_level(ch->skill_level[ENGINEERING_ABILITY]+1) - exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) );
		xpgain = UMIN( obj->cost*300 ,( exp_gain(ch->skill_level[ENGINEERING_ABILITY], gsn_makeholster) ) );
		gain_exp(ch, xpgain, ENGINEERING_ABILITY);
		ch_printf( ch , "You gain %d engineering experience.\n\r", xpgain );
	}
	learn_from_success( ch, gsn_makeholster );
}
...
void do_gemcutting( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		if( arg[0] == '\0' )
		{
			send_to_char( "Usage: Gemcutting <name>\n\r&w", ch );
			return;
		}
		...
		for( obj = ch->last_carrying; obj; obj = obj->prev_content )
		{
			if( obj->item_type == ITEM_TOOLKIT )
				checktool = TRUE;
			if( obj->item_type == ITEM_CRYSTALORE )
				checkore = TRUE;
		}

		if( !checktool )
		{
			send_to_char( "&RYou need a toolkit to cut gems.\n\r", ch );
			return;
		}

		if( !checkore )
		{
			send_to_char( "&RYou need some crystal ore to cut a gem out of.\n\r", ch );
			return;
		}
		...
		schance = IS_NPC( ch ) ? ch->top_level : ( int )( ch->pcdata->learned[gsn_gemcutting] * 100 / (ch->pcdata->learned[gsn_gemcutting] + 1) );

		if( number_percent(  ) < schance )
		{
			send_to_char( "&GYou begin the long process of gemcutting.\n\r", ch );
			act( AT_PLAIN, "$n takes $s tools and a piece of ore and begins to work.", ch, NULL, argument, TO_ROOM );
			add_timer( ch, TIMER_DO_FUN, IS_IMMORTAL( ch ) ? 1 : 25, do_gemcutting, 1 );
			ch->dest_buf = str_dup( arg );
			return;
		}
		send_to_char( "&RYou can't seem to figure out where to begin.\n\r", ch );
	...
	level = IS_NPC( ch ) ? ch->top_level : ( int )( ch->pcdata->learned[gsn_gemcutting] );
	...
	for( obj = ch->last_carrying; obj; obj = obj->prev_content )
	{
		if( obj->item_type == ITEM_TOOLKIT )
			checktool = TRUE;
		if( obj->item_type == ITEM_CRYSTALORE && !checkore )
		{
			ore = obj;
			checkore = TRUE;
	...
	{
		send_to_char( "&RWell, thats a ruined crystal. Shame.\n\r", ch );
		do_emote( ch, "grumbles and throws a wasted crystal away." );
		learn_from_failure( ch, gsn_gemcutting );
		return;
	}
...
//		xpgain = UMIN( obj->cost * 200, ( exp_level( ch->skill_level[ENGINEERING_ABILITY] + 1 ) - exp_level( ch->skill_level[ENGINEERING_ABILITY] ) ) ) / 2;
		xpgain = UMIN(obj->cost * 200, (exp_gain(ch->skill_level[ENGINEERING_ABILITY], gsn_gemcutting)));
		gain_exp( ch, xpgain, ENGINEERING_ABILITY );
		ch_printf( ch, "You gain %d engineering experience.\n\r", xpgain );
	}
...
void do_reinforcements( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = (int) (ch->pcdata->learned[gsn_reinforcements] * 100 / (ch->pcdata->learned[gsn_reinforcements] + 1));
		if ( number_percent( ) < chance )
		{
...
void do_postguard( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = (int) (ch->pcdata->learned[gsn_postguard] * 100 / (ch->pcdata->learned[gsn_postguard] + 1));
		if ( number_percent( ) < chance )
		{
			send_to_char( "&GYou begin making the call for reinforcements.\n\r", ch);
			act( AT_PLAIN, "$n begins issuing orders int $s comlink.", ch,
...
void do_disguise( CHAR_DATA *ch, char *argument )
{
	...
	chance = (int) (ch->pcdata->learned[gsn_disguise] * 100 / (ch->pcdata->learned[gsn_disguise] + 1));
...
void do_deception( CHAR_DATA *ch, char *argument )
{
	...
	chance = (int) (ch->pcdata->learned[gsn_deception] * 100 / (ch->pcdata->learned[gsn_deception] + 1));
...
void do_first_aid( CHAR_DATA *ch, char *argument )
{
	...
	if ( heal > (ch->pcdata->learned[gsn_first_aid] * 100 / (ch->pcdata->learned[gsn_first_aid] + 1)) )
	{
...
void do_snipe( CHAR_DATA *ch, char *argument )
{
	...
	chance = IS_NPC(ch) ? 100
			: (int)  (ch->pcdata->learned[gsn_snipe] * 100 / (ch->pcdata->learned[gsn_snipe] + 1)) ;
...
void do_throw( CHAR_DATA *ch, char *argument )
{
   ...
   if( obj_extracted(obj) )
	...
	else
	{
		...
		if ( IS_NPC(ch) || number_percent( ) < (ch->pcdata->learned[gsn_throw] * 100 / (ch->pcdata->learned[gsn_throw] + 1)) )
		{
...
void do_taxi(CHAR_DATA *ch, char *argument )
{
	...
	// First check to see if player is on a landing pad.
	if(!IS_SET( ch->in_room->room_flags, ROOM_CAN_LAND ))
	{
		send_to_char("&RYou must be on a landing pad to call a taxi.", ch);
		return;
	}

	// Go through each room looking for a landing pad.
	ROOM_INDEX_DATA *room = NULL;
	for(room = ch->in_room->area->first_room; room; room = room->next_in_area)
	{
		// Check if room is a landing pad.
		if(IS_SET(room->room_flags, ROOM_CAN_LAND))
		{
			//Search room for public ships.
			SHIP_DATA *ship = NULL;
	...
	// If a suitable ship hasn't been found, import a ship from another system.
	if(!sh)
	{
		AREA_DATA *area = NULL;
	...
	if(sh)	// If a ship has been found.
	{
		// Check if player has enough credits.
		if(ch->gold < hailFee)
		{
			send_to_char("&RYou don't have enough credits to hail a shuttle.\n\r", ch);
			return;
		}
		ch->gold = ch->gold - hailFee;
		boost_economy(ch->in_room->area, hailFee);

		// Transfer the ship.
		sh->shipstate = SHIP_READY;
		sh->location = ch->in_room->vnum;
		extract_ship( sh );
		ship_to_room( sh , sh->location );

		sh->lastdoc = sh->location;
		sh->shipstate = SHIP_DOCKED;
		if (sh->starsystem)
			ship_from_starsystem( sh, sh->starsystem );
		save_ship(sh);

		// Tell the player and room all that has happened.
		char buf[MAX_STRING_LENGTH];
		sprintf(buf, "&GYou hail a shuttle and pay the %i credit fee.\n\r", hailFee);
		send_to_char(buf, ch);

		sh->hatchopen = TRUE;
		buf[0] = '\0';
		sprintf( buf , "&YThe hatch on %s slides open.\n\r" , sh->name);
		echo_to_room( AT_YELLOW , get_room_index(sh->location) , buf );
	}
	else	// No free taxis have been found.
	{
		send_to_char("&RThere are no available shuttles to answer your call.\n\r", ch);
	}
}
...
//	xp = UMIN( amount*10 , ( exp_level( ch->skill_level[SMUGGLING_ABILITY]+1) - exp_level( ch->skill_level[SMUGGLING_ABILITY])  )  );
	xp = UMIN( amount*10 , ( exp_gain(ch->skill_level[SMUGGLING_ABILITY], gsn_beg)  )  );
...
void do_hijack( CHAR_DATA *ch, char *argument )
{
	...
	chance = IS_NPC(ch) ? 0
			: (int)  (ch->pcdata->learned[gsn_hijack] * 100 / (ch->pcdata->learned[gsn_hijack] + 1)) ;
	x = number_percent( );
	if ( x > chance )
	{
	...
	chance = IS_NPC(ch) ? 0
			: (int)  (ch->pcdata->learned[gsn_shipsystems] * 100 / (ch->pcdata->learned[gsn_shipsystems] + 1)) ;
	if ( number_percent( ) < chance )
	{
...
void do_special_forces ( CHAR_DATA *ch , char *argument )
{
	...
	default:
		...
		chance = (int) (ch->pcdata->learned[gsn_specialforces] * 100 / (ch->pcdata->learned[gsn_specialforces] + 1));
		if ( number_percent( ) < chance )
		{
			send_to_char( "&GYou begin making the call for reinforcements.\n\r", ch);
			act( AT_PLAIN, "$n begins issuing orders int $s comlink.", ch,
...
void do_elite_guard ( CHAR_DATA *ch , char *argument )
{
	...
	default:
		...
		chance = (int) (ch->pcdata->learned[gsn_eliteguard] * 100 / (ch->pcdata->learned[gsn_eliteguard] + 1));
		if ( number_percent( ) < chance )
		{
			send_to_char( "&GYou begin making the call for reinforcements.\n\r", ch);
...
void do_add_patrol ( CHAR_DATA *ch , char *argument )
{
	...
	default:
		...
		chance = (int) (ch->pcdata->learned[gsn_addpatrol] * 100 / (ch->pcdata->learned[gsn_addpatrol] + 1));
		if ( number_percent( ) < chance )
		{
			send_to_char( "&GYou begin making the call for reinforcements.\n\r", ch);
			act( AT_PLAIN, "$n begins issuing orders int $s comlink.", ch,
...
void do_smalltalk ( CHAR_DATA *ch , char *argument )
{
	...
	if ( percent - ch->skill_level[POLITICIAN_ABILITY] + victim->top_level > (ch->pcdata->learned[gsn_smalltalk] * 100 / (ch->pcdata->learned[gsn_smalltalk] + 1))  )
	{
		/*
		 * Failure.
		 */
...
void do_propeganda ( CHAR_DATA *ch , char *argument )
{
	...
	if ( percent - get_curr_cha(ch) + victim->top_level > (ch->pcdata->learned[gsn_propeganda] * 100 / (ch->pcdata->learned[gsn_propeganda] + 1))  )
	{
...
void do_bribe ( CHAR_DATA *ch , char *argument )
{
	...
	if ( percent - amount + victim->top_level > (ch->pcdata->learned[gsn_bribe] * 100 / (ch->pcdata->learned[gsn_bribe] + 1))  )
		return;
...
//		amount = UMIN( amount ,( exp_level(ch->skill_level[POLITICIAN_ABILITY]+1) - exp_level(ch->skill_level[POLITICIAN_ABILITY]) ) );
		amount = UMIN( amount ,( exp_gain(ch->skill_level[POLITICIAN_ABILITY], gsn_bribe) ) );
...
void do_seduce ( CHAR_DATA *ch , char *argument )
{
	...
	level = IS_NPC(ch) ? ch->top_level : (ch->pcdata->learned[gsn_seduce] * 100 / (ch->pcdata->learned[gsn_seduce] + 1));
...
void do_mass_propeganda ( CHAR_DATA *ch , char *argument )
{
	...
	if ( number_percent() < (ch->pcdata->learned[gsn_masspropeganda] * 100 / (ch->pcdata->learned[gsn_masspropeganda] + 1))  )
	{
		for ( rch = ch->in_room->first_person; rch; rch = rch->next_in_room )
		{
			if ( rch == ch )
...
void do_repair( CHAR_DATA *ch, char *argument)
{
	...
	chance = IS_NPC(ch) ? ch->top_level : (int)(ch->pcdata->learned[gsn_repair] * 100 / (ch->pcdata->learned[gsn_repair] + 1));
...
void do_makeduallightsaber( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makeduallightsaber] * 100 / (ch->pcdata->learned[gsn_makeduallightsaber] + 1));
		if ( number_percent( ) < chance )
		{
			send_to_char( "&GYou begin the long process of crafting a lightsaber.\n\r", ch);
			act( AT_PLAIN, "$n takes $s tools and a small oven and begins to work on something.", ch,
...
//		xpgain = UMIN( obj->cost*50 ,( exp_level(ch->skill_level[FORCE_ABILITY]+1) - exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) );
		xpgain = UMIN( obj->cost*50 ,( exp_gain(ch->skill_level[FORCE_ABILITY], gsn_makeduallightsaber) ) );
		gain_exp(ch, xpgain, FORCE_ABILITY);
		ch_printf( ch , "You gain %d force experience.", xpgain );
	}
...
void do_makepike( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makepike] * 100 / (ch->pcdata->learned[gsn_makepike] + 1));
		if ( number_percent( ) < chance )
		{
	...
	send_to_char( "&GYou finish your work and hold up your newly created force pike.&w\n\r", ch);
        ...
        ch_printf(ch, "&WYour pike is of &R%s &wquality.\n\r", word);
...
//		xpgain = UMIN( obj->cost*20 ,( exp_level(ch->skill_level[ENGINEERING_ABILITY]+1) - exp_level(ch->skill_level[ENGINEERING_ABILITY]) ) );
		xpgain = UMIN( obj->cost*20 ,( exp_gain(ch->skill_level[ENGINEERING_ABILITY], gsn_makepike) ) );
		gain_exp(ch, xpgain, ENGINEERING_ABILITY);
		ch_printf( ch , "You gain %d engineering experience.", xpgain );
	}
...
void do_makebug( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makebug] * 100 / (ch->pcdata->learned[gsn_makebug] + 1));
		if ( number_percent( ) < chance )
		{
...
void do_makebeacon( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makebeacon] * 100 / (ch->pcdata->learned[gsn_makebeacon] + 1));
		if ( number_percent( ) < chance )
		{
...
void do_checkbeacons( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level : (int)(ch->pcdata->learned[gsn_checkbeacons] * 100 / (ch->pcdata->learned[gsn_checkbeacons] + 1));
...
void do_nullifybeacons(CHAR_DATA *ch, char *argument)
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level : (int)(ch->pcdata->learned[gsn_nullifybeacons] * 100 / (ch->pcdata->learned[gsn_nullifybeacons] + 1));
...
void do_makebinders( CHAR_DATA *ch, char *argument )
{
	...
	default:
		...
		chance = IS_NPC(ch) ? ch->top_level
				: (int) (ch->pcdata->learned[gsn_makebinders] * 100 / (ch->pcdata->learned[gsn_makebinders] + 1));
		if ( number_percent( ) < chance )
		{
...
void do_battle_command(CHAR_DATA *ch, char *argument)
{
	...
	chance = IS_NPC(ch) ? ch->top_level : (int)(ch->pcdata->learned[gsn_battle_command] * 100 / (ch->pcdata->learned[gsn_battle_command] + 1));
...
void do_slicelock (CHAR_DATA *ch, char *argument)
{
        char arg[MAX_INPUT_LENGTH];
        CHAR_DATA *gch;
        EXIT_DATA *dexit;
        ...
        if (checkdata == FALSE)
        {
                send_to_char("You need a datapad to slice this lock!\r\n",ch);
                return;
        }
        ...
        if (arg[0] == '\0')
        {
               send_to_char("&RSyntax: slicelock <direction> \r\n",ch);
        }
        ...
        if (ch->mount)
        {
                send_to_char("You can't do that while your mounted!!\r\n",ch);
                return;
        }

        WAIT_STATE(ch, skill_table[gsn_slicelock]->beats);

        for (gch = ch->in_room->first_person; gch; gch = gch->next_in_room)
        {
                if (IS_NPC(gch) && IS_AWAKE(gch) && ch->skill_level[SLICER_ABILITY] < gch->top_level)
                {
        ...
        if ((dexit = find_door(ch,arg,TRUE)) != NULL)
        {
                EXIT_DATA *dexit_rev;

                if (!IS_SET(dexit->exit_info, EX_CLOSED))
                {
                        send_to_char("It's not closed!\r\n",ch);
                        return;
                }

                if(!IS_SET(dexit->exit_info, EX_KEYPAD))
                {
                        send_to_char("&RThats not a keypad lock! Impossible to slice it!\n",ch);
                        return;
                }

                if (dexit->key < 0)
                {
                        send_to_char("It doesn't need to be picked!!\r\n",ch);
                        return;
                }
                
                if (!IS_SET(dexit->exit_info, EX_LOCKED))
                {
                        send_to_char("It's not locked!\r\n",ch);
                        return;
                }
         
                if (IS_SET(dexit->exit_info, EX_PICKPROOF))
                {
                        send_to_char("You failed to slice the lock!\r\n",ch);
                        learn_from_failure(ch,gsn_slicelock);
                        return;
                }
               chances = IS_NPC(ch) ? ch->top_level
                        : (int) (ch->pcdata->learned[gsn_slicelock] * 100 / (ch->pcdata->learned[gsn_slicelock] + 1));

                if (number_percent( ) < chances)
                {
                        REMOVE_BIT(dexit->exit_info, EX_LOCKED);
                        send_to_char("*The door clicks*\r\n",ch);
                        act (AT_ACTION, "$n slices the $d!",ch,NULL,dexit->keyword,TO_ROOM);
                        learn_from_success(ch, gsn_slicelock);
                }
                else
                {
                        send_to_char("You failed to slice the lock!",ch);
                        learn_from_failure(ch,gsn_slicelock);
                }
                if ((dexit_rev = dexit->rexit) != NULL && dexit_rev->to_room == ch->in_room)
                {
                        REMOVE_BIT(dexit_rev->exit_info, EX_LOCKED);
                }

                check_room_for_traps(ch,TRAP_PICK | trap_door[dexit->vdir]);
                return;
        }
}
