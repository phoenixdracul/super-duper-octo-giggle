 ...
 * 						        Planets Module								*
 * 																			*
 *  Provides functions for loading of planets and planet-related commands.	*
 * 																			*
 +--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--+
 *																			*
...
void do_setplanet( CHAR_DATA *ch, char *argument )
{
   ...
   if( !strcmp( arg2, "shipcapacity" ) )
   {
    ...
    if( !strcmp( arg2, "base_value" ) )	// TODO
	{
		int bval;
...
{   
    ...
    ch_printf( ch, "&WCoordinates: &w[ &G%d&W, &G%d&W, &G%d &w]\n\r", planet->x, planet->y, planet->z);
