...
/* Obsolete. Replaced with act. - Boran */
...
/* This is its own function so that mana is ALWAYS standardized */
void roll_mana(CHAR_DATA *ch)
{
	ch->max_mana = (ch->force_control + ch->force_sense + ch->force_alter) * 8;
	if(ch->max_mana < 0)
		ch->max_mana = ch->max_mana * -1;
...
void roll_force(CHAR_DATA* victim, char* type)
{
	if(!str_cmp(type, "jedi"))
		...
		victim->force_type = 2;
	else if(!str_cmp(type, "custom"))
		victim->force_type = victim->force_type;
	else
		victim->force_type = 3;

	// Only roll perm_frc again if it's zero.
	if(victim->perm_frc == 0)
		victim->perm_frc = number_range(3, 6)+number_range(1, 6)+number_range(1, 6);
	...
	victim->force_level_status = 1;
	roll_mana(victim);

	// TODO When implementing the final system, do not automatically set all skills at 20%
	int i;
	for(i = 0; i < MAX_FORCE_SKILL; ++i)
		victim->force_skill[i] = 20;
...
void do_fsstat(CHAR_DATA *ch,char *argument)
{
	FORCE_SKILL *fskill;
	...
	if(!fskill)
	{
		...
		send_to_char("USAGE: fsstat <skill> [mesg]\r\n",ch);
		return;
	}
...
void do_fsset(CHAR_DATA *ch, char *argument)
{
	FORCE_SKILL *fskill;
	char arg1[MAX_STRING_LENGTH];
	...
	{
	default:
		bug( "do_fsset: illegal substate", 0 );
		return;
	...
	case SUB_NONE:
		send_to_char("USAGE: fsset <skill> <field> [<value>]\r\n",ch);
		send_to_char("    type: jedi sith general\r\n",ch);
		send_to_char("	  control, sense, alter, disabled\n\r", ch);
		send_to_char("	  notskill, mastertrain, cost, level\n\r\n\r", ch);
		return;
	case SUB_FORCE_CH0:
		fskill = ch->spare_ptr;
